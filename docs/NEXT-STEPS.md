...
2. ☐ **Round 2 at Adam's 3PM codex window** — `dev/model-qa/regen-v3/round2/` (cosmic-r2 15,
   gloom-r2 3, fixes-r2 9); then gate → slice → tag the returns (same pipeline as this session).
...
4. ☐ **Retro-tag the committed corpus** to the SPRITE-TAGS schema + check committed ash for the
   painterly drift (lane ash passed clean — Adam's memory points at older sheets).
