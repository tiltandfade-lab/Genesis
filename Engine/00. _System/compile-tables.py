...
for f in files:
    ...
    for context,rows in split_blocks(body.splitlines()):
        ...
        if EMIT:
            band_col=legs_col=arch_col=grants_col=motif_col=footprint_col=None
            ...
            if hdrs:
                for ci,h in enumerate(CELL(c).lower() for c in hdrs[0].strip('|').split('|')):
                    ...
                    elif h.strip()=='map footprint': footprint_col=ci  # TERRAIN-PROGRAM.md M8 / BATTLEMAP.md
                        # §3b ("a `Map Footprint` column ... DM-only column, compiler carries it — the
                        # Legs/Pool precedent"). Unscoped (any table), matching band/legs/pool's own
                        # style — checked: no OTHER compiled table's header cell is literally "Map
                        # Footprint" (audited via a read-only header-collision sweep against every
                        # dice-table block in ROOTS at fix time), and BATTLEMAP.md §3b's own plan
                        # (dungeon feature/hazard tables, urban scene frames) explicitly reuses this SAME
                        # header spelling, so leaving it table-id-unscoped is forward-compatible with
                        # that follow-up rather than a footgun.
            ...
            # TERRAIN-PROGRAM.md M8: `Map Footprint` is the same kind of DM-only tag column again —
            # excluded from narration text + row[5] cols, emitted as row[10] ONLY when present (every
            # table except wilderness-tactical-terrain today stays byte-identical). It is deliberately
            # NOT slotted into row[6]/row[7] (which would collide with legs/pool on any FUTURE table
            # that authors both tag families) — instead `foot_col` forces the SAME padding discipline
            # tag_cols/skin_cols already use, so row[10] means "footprint" on every compiled table,
            # unconditionally, the same global guarantee row[6]="legs"/row[8]="grants" already carry.
            tag_cols = legs_col is not None or arch_col is not None
            skin_cols = grants_col is not None or motif_col is not None
            foot_col = footprint_col is not None
            exclude={0,band_col,legs_col,arch_col,grants_col,motif_col,footprint_col}
            erows=[]
            for lo,hi,cells in parsed:
                ...
                cols=[c for ci,c in enumerate(cells) if ci!=0 and ci!=legs_col and ci!=arch_col and ci!=grants_col and ci!=motif_col and ci!=footprint_col]
                row=[lo,hi,band,txt,None,cols]
                if tag_cols or skin_cols or foot_col:
                ...
                if skin_cols or foot_col:
                    ...
                    row+=[grants,motif]
                if foot_col:
                    footprint=cells[footprint_col] if footprint_col<len(cells) else ""
