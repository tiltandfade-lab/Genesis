---
id: dungeon-loot-budget-t4
type: table
domain: Session Mechanics / Dungeons
status: source
table_class: Fork
player_facing: reveal
voice_critical: false
---

> ⛔ **DEFERRED — Tier 4 (Lv 17–20) ships with the expansion (docs/TIER-SCOPE.md).** Authored but inert: no in-game generator reaches it (this version caps at Tier 2 / L10). Do not wire.

> [!info] T4 Loot Budget Reference (Lv 17–20)
> Based on DMG 2024 tier targets (per-party cumulative across the tier): 5 Common, 7 Uncommon, 6 Rare, 5 Very Rare, 3 Legendary. Budget below is one dungeon's share (same per-dungeon-share logic as T1/T2; tunable in play). The generator computes from segment count; deepest rooms get rarest items; boss gets the top-rarity slot.

| **Dungeon Size** | **Segments** | **Common** | **Uncommon** | **Rare** | **Very Rare** | **Legendary** |
| ---------------- | ------------ | ---------- | ------------ | -------- | ------------- | ------------- |
| Small | 5–8 | 1 | 2 | 2 | 1 | 0 |
| Medium | 9–12 | 2 | 2 | 2 | 2 | 1 |
| Large | 13+ | 2 | 3 | 2 | 2 | 1 |

> **Distribution:** Legendary → Boss room. Very Rare → deepest cluster. Rare → mid-depth. Uncommon → shallow rooms. Common → remaining. All other rooms receive coin only (see Dungeon Loot - Coin Cache). Artifacts remain story placements, not budgeted (see Dungeon Loot - Artifact) — at T4 a single story Artifact may anchor a capstone dungeon.
^dungeon-loot-budget-t4
