...
**Adam's ledger (rulings wanted):**
1. **Urban→rural tier-0 banding** — an unshopped city node mints rural animal ecology (plough
   oxen on city streets) and the env cast FREEZES that way (idempotency guard). Documented
   deliberate tradeoff (lodging tier is the only urban signal today). Keep, or give nodes a real
   urban signal?
2. **Herd/bird witness channels** — `outcome:move-zone` (herd) and `npc-life` (bird) writers
   stamp no location, so those channels stay verify-only. Every scope has ≥1 live channel now;
   stamping those writers is a small follow-up unit if wanted.
