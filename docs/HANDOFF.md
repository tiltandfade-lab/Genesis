...
## This session (2026-06-21 — latest) — DM Bridge + chat-first UI + Ivalice reskin [Claude Code]
...
## Next move
**Lanes B (chat-first UI) and C (DM Bridge) are BUILT** (see latest-session block), and the UI is now the **Ivalice reskin**. **Focus shifts to lane A — game mechanics (advancement).** Adam is starting a fresh session on mechanics.
...
**(A) Advancement — the meat & potatoes.** Specs locked (`EVENT-CONTRACT.md` / `ADVANCEMENT.md` / `DIFFICULTY.md` / `COMBAT.md`). Step 1 (`CLASS_PROGRESSION`, levels 1–20) ☑ done. **The `applyEvent(w,e)` runtime in `src/world/dm.js` is now built — that's the event-contract spine; advancement plugs straight into it.** Do next:
  1. **Decide the XP threshold curve** — SRD 5.2.1 numbers vs a compressed custom curve (exploration-heavy sessions may pace oddly on SRD XP; `ADVANCEMENT.md` flags this as the call to make *before* wiring leveling). Author as a compile-ready markdown table → JSON.
