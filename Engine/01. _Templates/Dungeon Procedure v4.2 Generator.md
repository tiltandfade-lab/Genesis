> [!info] Dungeon Procedure v4.2 Generator
...
>
> v4.2 changes from v4.1: Full loot system rewrite aligned to 5e 2024 DMG. Tier-aware budget (T1/T2) keyed to dungeon size. Magic items distributed by BFS depth — deepest rooms first, boss always carries the top item. Four rarity tiers: Common, Uncommon, Rare, Very Rare. All rooms receive coin scaled to tier and depth. Enemy encounters note enemy loot (coin + worn gear, separate from magic budget). Old tables archived.

<%*
// =============================================================================
// DUNGEON SKETCH GENERATOR — Arcana Engine v0.25 · Dungeon Procedure v4.2
// v4.2: Loot system rewrite. Tier-aware budget (T1/T2) from DMG 2024.
//       Depth-sorted magic item assignment — boss gets top rarity, deepest
//       non-boss rooms get next best. All rooms receive coin. Enemy encounters
//       note enemy loot separately (no draw from budget). Four rarity tiers:
//       Common, Uncommon, Rare, Very Rare. Old Legendary/Outlandish archived.
...
// ─── LOOT BUDGET ─────────────────────────────────────────────────────────────
// v4.2: Tier-aware budget aligned to DMG 2024. Items distributed by BFS depth.
// Boss always gets top rarity. All rooms receive coin regardless of magic slot.

function computeBudget(segCount, tier) {
  if (tier === "T2") {
    if (segCount >= 13) return { common: 4, uncommon: 6, rare: 2, veryRare: 1 };
    if (segCount >= 9)  return { common: 3, uncommon: 5, rare: 1, veryRare: 0 };
    return                     { common: 2, uncommon: 3, rare: 0, veryRare: 0 };
  }
  // T1 default
  if (segCount >= 13) return { common: 3, uncommon: 2, rare: 1, veryRare: 0 };
  if (segCount >= 9)  return { common: 2, uncommon: 2, rare: 0, veryRare: 0 };
  return                     { common: 1, uncommon: 1, rare: 0, veryRare: 0 };
}

function assignLootByDepth(budget, bfsOrder, bfsDepth, finaleId) {
  // Build priority deck: best rarity first (no shuffle — depth assignment handles ordering)
  const deck = [];
  ...
  // Sort non-finale rooms by depth descending — deepest gets best items
  const nonFinale = bfsOrder.filter(id => id !== finaleId);
  nonFinale.sort((a, b) => (bfsDepth[b] || 0) - (bfsDepth[a] || 0));

  const lootByNode = {};
  // Boss always gets the top deck item
  lootByNode[finaleId] = deck.length > 0 ? deck.shift() : null;
  // Distribute remainder by depth
  for (const id of nonFinale) {
    lootByNode[id] = deck.length > 0 ? deck.shift() : null;
  }
  return lootByNode;
}

function computeCoin(tier, depth, isFinale) {
  const roll = (n, s) => Array.from({length: n}, () => Math.floor(Math.random() * s) + 1).reduce((a,b)=>a+b,0);
  if (tier === "T2") {
    if (isFinale)   { const gp = roll(2,6)*50; const gems = roll(1,4); return `${gp} gp + ${gems} gem${gems>1?"s":""} (50 gp ea)`; }
    if (depth >= 4) return `${roll(2,6)*10} gp`;
    if (depth >= 2) return `${roll(2,6)*5} gp`;
    return `${roll(2,6)} gp`;
  }
  // T1
  if (isFinale)   { const gp = roll(2,6)*5; return `${gp} gp + 1 gem (10 gp)`; }
  if (depth >= 4) return `${roll(2,6)} gp`;
  if (depth >= 2) return `${roll(1,6)} sp, ${roll(1,4)} gp`;
  return `${roll(2,6)*10} cp`;
}

function haulSummary(budget) {
  const parts = [];
  if (budget.veryRare > 0) parts.push(`${budget.veryRare} Very Rare`);
  if (budget.rare     > 0) parts.push(`${budget.rare} Rare`);
  if (budget.uncommon > 0) parts.push(`${budget.uncommon} Uncommon`);
  if (budget.common   > 0) parts.push(`${budget.common} Common`);
  parts.push("Coin in all rooms");
  return parts.join(" · ");
}

async function resolveLootSlot(rarity) {
  if (rarity === "very-rare") { const [n, d] = await pick("Dungeon Loot - Very Rare", 1, 2); return { label: "[Very Rare]", name: n, desc: d }; }
  if (rarity === "rare")      { const [n, d] = await pick("Dungeon Loot - Rare",      1, 2); return { label: "[Rare]",      name: n, desc: d }; }
  if (rarity === "uncommon")  { const [n, d] = await pick("Dungeon Loot - Uncommon",  1, 2); return { label: "[Uncommon]",  name: n, desc: d }; }
  if (rarity === "common")    { const [n, d] = await pick("Dungeon Loot - Common",    1, 2); return { label: "[Common]",    name: n, desc: d }; }
  return null;
}

async function buildLootBlock(lootSlot, depth, isFinale, tier, hasEnemyEncounter) {
  const coinStr = computeCoin(tier, depth, isFinale);
  const lines = [];
  if (lootSlot) {
    const item = await resolveLootSlot(lootSlot);
    if (item) {
      const bossTag = isFinale ? " *(boss carries this)*" : "";
      lines.push(`- **${item.label}** ${item.name} — *${item.desc}*${bossTag}`);
    }
  }
  lines.push(`- **Coin:** ${coinStr}`);
  if (hasEnemyEncounter && !isFinale) {
    lines.push(`- **Enemy Loot:** Coin + worn gear by role — see [[Dungeon Loot - Enemy]]`);
  }
  return `**Loot**\n${lines.join("\n")}`;
}
...
async function generateRoomNote(roomNum, node, exits, areaType, dims, side, isEntry, threatCtx, lootSlot, depth, dungeonName, tier) {
  ...
  const loot      = await buildLootBlock(lootSlot, depth, false, tier, encounter.includes("**Encounter: Enemy**"));
...
async function generateFinaleNote(roomNum, node, exits, areaType, dims, side, depth, threatCtx, lootSlot, bossAffinity, revelAffinity, dungeonName, tier) {
  const [lighting, lightFlavor] = await pick("Dungeon Lighting", 1, 2);
  const [sensory]               = await pick("Dungeon Sensory",  1);
  const [scene]                 = await pick("Dungeon Scene",    1);
  const [d1] = await pick("Dungeon Set Dressing", 1);
  ...
  const [devName, devSit, devMisread, devLev] = await pick("Dungeon Narrative Device", 1, 2, 3, 4);
  const conns  = await buildConnections(exits);
  const loot   = await buildLootBlock(lootSlot, depth, true, tier, false);
  const secret = await buildSecret();
  const exitWord = exits.length === 1 ? "exit" : "exits";
...
const lootBudget = computeBudget(segCount, tier);
const finaleId   = bfsOrder.find(id => nodeMap[id]?.isFinale) || bfsOrder[bfsOrder.length - 1];
const lootByNode = assignLootByDepth(lootBudget, bfsOrder, bfsDepth, finaleId);
...
for (const nodeId of bfsOrder) {
  ...
  let noteContent;
  if (node.isFinale) {
    noteContent = await generateFinaleNote(roomNum, node, exits, areaType, dims, side, bfsDepth[nodeId], threatCtx, lootByNode[nodeId], bossAffinity, revelAffinity, dungeonName, tier);
  } else {
    noteContent = await generateRoomNote(roomNum, node, exits, areaType, dims, side, isEntry, threatCtx, lootByNode[nodeId], bfsDepth[nodeId], dungeonName, tier);
  }
