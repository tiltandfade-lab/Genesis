/* dev/model-qa/creatures/halforc-warlock.js — RACE×CLASS bespoke starter (POLISH-WAVE-1 §F4, wave B).
   The FIXED half-orc race (heavy/broad frame, gray-green skin, heavy jaw + tusk nubs + jutting brow)
   wearing the WARLOCK kit (warlock.js signature: an open GRIMOIRE authored first cradled on the raised
   off forearm, a raised claw/invoking hand, a long layered dark ROBE, a glowing eye-AMULET at the
   chest). A hulking orcish pact-caster; the tusks + brow read on the cropped-hair head. EYELESS. One
   whole-object function, no anchors; figure faces +z front. */
import { THREE, V, quad, tube, stack, ring, stitch, capFan } from '../probe-lib.js';

export function buildHalfOrcWarlock(){
  const P = {
  ...
  const SPINE_A = V(-0.470, 0.860, 0.330), SPINE_B = V(-0.190, 0.895, 0.420);
  ...
  /* TRUNK — hunched (leans forward as y rises), broad half-orc */
  {
    const bands=[
      {y:L.hipY,   rx:0.230, rz:0.180, cz:0.0,   hex:P.underDk},
      {y:L.waistY, rx:0.205, rz:0.160, cz:0.010, hex:P.under},
      {y:L.ribY,   rx:0.246, rz:0.184, cz:0.024, hex:P.under},
      {y:L.chestY, rx:0.276, rz:0.198, cz:0.042, hex:P.robeDk},
      {y:L.shldY,  rx:0.290, rz:0.185, cz:0.058, hex:P.robeDk},
      {y:L.neckY,  rx:0.150, rz:0.142, cz:0.062, hex:P.skinDk},
    ];
    stack(bands, 8, {capTop:{hex:P.skinDk, lift:0.004}});
  }

  /* LONG LAYERED ROBE — floor-length outer shell (broad half-orc) */
  stack([
  ...
  /* HEAD — inherited half-orc skull, slight +z on the hunched neck. EYELESS. */
  const HEAD_Z=0.040;
  {
    ...
    const rings=bands.map(b=>ring(V(0,b.y,HEAD_Z+0.014), V(0,1,0), b.rx, b.rz, n, ph));
    ...
    capFan(rings[3], V(0, L.headTopY, HEAD_Z+0.010), P.skinDk);
    for(const s of [-1,1]){
      const bx=s*0.044, by=L.jawY-0.006, bz=HEAD_Z+0.140;
    ...
    /* cropped dark hair cap */
    const hb=[
      {y:L.browY+0.010, rx:0.136, rz:0.122, hex:P.hairDk},
      {y:L.crownY+0.004,rx:0.112, rz:0.100, hex:P.hair},
    ];
    const hr=hb.map(b=>ring(V(0,b.y,HEAD_Z+0.006), V(0,1,0), b.rx, b.rz, n, ph));
    stitch(hr, b=>hb[b].hex, {0:[1,2,3,4,5,6]});
    capFan(hr[1], V(0, L.headTopY+0.010, HEAD_Z-0.004), P.hair);
  }

  /* LEFT ARM — raised, forearm derived under the grimoire spine */
  {
    const S=V(-L.shoulderX, L.shldY-0.02, 0.05);
    const E=V(-0.445, 0.800, 0.230);
    tube(S,E,0.098,0.076,6,P.robe);
    tube(E, FOREARM_GRIP.clone().addScaledVector(SPINE_DIR,-0.03), 0.072,0.058,6,P.robeLt,{capB:{hex:P.skinDk}});
  ...
  const CDIR = V(0.35, 0.30, 0.55).normalize();
  {
    ...
    tube(E2,W2,0.068,0.052,6,P.robeLt,{capB:{hex:P.skinDk}});
    tube(W2, CLAW, 0.052,0.040,6,P.skin,{capA:{hex:P.skin},capB:{hex:P.skin}});
    const up=V(0,1,0);
    ...
    for(const k of [-2.2,-0.9,0.9,2.2]){
      ...
      tube(base,knuckle,0.014,0.011,4,P.skin);
      tube(knuckle,tip,0.011,0.006,4,P.skinDk,{capB:{hex:P.skinDk}});
    }
  }
