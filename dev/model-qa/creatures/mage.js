/* dev/model-qa/creatures/mage.js — the robed spellcaster landmark table (whole-object probe).
   Same whole-object grammar as humanoid.js: the ENTIRE creature is one function of shared
   primitives, every vertex in one model frame, no anchors. The staff is authored first so the
   casting fist is fitted to the shaft axis (grip true by construction). Silhouette-first: a
   floor-length robe (no exposed legs), wide bell sleeves, a long beard, and a pointed hat —
   the read that separates "mage" from "fighter" even under a flat PS1 tint. */
