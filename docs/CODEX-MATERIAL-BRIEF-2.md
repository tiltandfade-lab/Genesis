...
- Taste cards per the GP-MM-STONE-V001 pattern (reuse `dev/material-cards/`): tile · 3×3 seam ·
  ...
  existing stone candidates stay live — do not re-author them, but do not use their earlier
  material-first construction as precedent for new roster entries.
- Retain at least one explicit negative-control card per relevant family (too-small scale,
  ...
  <!-- merge synthesis 2026-07-25 (singular close): the sprite-first card list + failure routing
  (Adam's 2026-07-24 binding override, ART-DIRECTION-CANON) composed with the production-contracts
  process law (harness generalization, rulings registry, PENDING labeling, negative controls) —
  the two lanes edited this paragraph in parallel; both halves were already-ruled law. -->
