...
## Execution and adjudication seam (built 2026-08-04)

Events remain the only executable mutation surface. A preceding conservative route merely decides
ordering:

- `local-fact`: render known state; no model and no event;
- `declared-mechanic`: apply a complete engine operation first and return a
  `mechanical-receipt/v1` to the narrator;
- `freeform-ruling`: the DM interprets intent/applicability/stakes, then proposes top-level events
  through this contract. This is the default for unknown or contextual free text.

The receipt envelope is immutable during the turn and contains `{schema,id,kind,engineOwned,
deltaOnly,accepted,request,result,before,after,settledEventTypes}`. `deltaOnly:true` means before/after
contain changed mechanical paths only; an omitted path is unchanged. An accepted receipt is canonical: a
TurnResponse event whose type is already in `settledEventTypes` is surfaced as
`settled-by-receipt` and not applied again. Exact short/long rest is the first built proof.

A TurnResponse may also include an optional `ruling` proposal with `understoodAction`, `ruling`,
`needsRoll`, `proposedCheck`, `stakes`, `outcomeBranches`, and `proposedEvents`. This object is
validated but **never executed**. Only the response's top-level `events[]` reach `applyEvent`.
Therefore explanation/deliberation cannot become a second state authority.
...
| `clock_advanced` | `{clockId, delta}` (aliases `id/faction→clockId`, `by/n→delta`; ids come from the digest's `powers[].clockId` / `fronts[].clockId`) | detected | DIFFICULTY (escalation) |
...
| `encounter_resolved` | `{foes:[{cr,victimClass}], method, objectiveRef?, outcome}` | declared→detected | ADVANCEMENT, DIFFICULTY; when an active walk segment exists, also stamps its persistent overlay `{encounterState:"resolved", encounterResolution:{method,outcome,objectiveRef}}` so the generated threat cannot narratively respawn |
...
| `combat_end` | `{outcome:resolved\|fled\|pc-fled\|surrender\|negotiated\|pc-dead\|aborted, method?}` | DETECTED (the last live foe drops — `cmMaybeAutoEnd`, fired off the `attack` case, `foe_action`'s self-damage paths, and `foe_morale`'s flee/surrender/rout application) or declared (every other outcome) | COMBAT-LIFECYCLE.md §3 — `combatOutcomeEvents()` prices only DOWNED foes (fled/surrendered foes pay zero XP); `fled` means the foes withdrew, while `pc-fled` means the player escaped; emits one `encounter_resolved` + one `kill` per downed foe (even on `pc-dead` — escalation is real, XP to a dead PC no-ops harmlessly); `GS.combat=null`. A live `GS.combat` also tears down defensively the moment the PC dies (§3c, `killCharacter`) so the bardo never opens behind a live tracker. On a morale-flee pursuit, emit `chase_start` BEFORE this event — `chaseInit` copies only the fid string, so `GS.chase` survives the teardown (§3d) |
...
| `item_transfer` | `{itemId, qty?, to:{kind:pc\|npc\|creature\|faction\|container\|corpse\|place\|object, ref, name?}, intent?:transfer\|entrust\|gift\|loan\|place\|confiscated\|stolen\|lost, note?}` | declared (DM after interpreting a freeform placement) or player (trusted structured affordance) | atomic active-PC-inventory → explicit-holder move. Omitted intent defaults to neutral `transfer`; only explicit involuntary intent creates claimed/lost legacy state and a recovery hook. Singleton `qty` may be omitted; stacks require a positive integer. Whole moves keep the instance id; partial moves keep the old id on the remainder and return a new moved id. PC/corpse destinations use their native item arrays; other holders use `w.itemCustody`; a destination PC still obeys the STR×30 hard cap. Clears equipped slots only when the whole instance leaves. Rejects invalid destination/quantity before mutation; refuses partial storied stacks (`storied-stack-unsplittable`). A linked Codex item folds the matching `item_claimed` transition in the same operation. Trusted declarations resolve before narration through `mechanical-receipt/v1`; replay is blocked. |
| `item_placed` | `{item:{name,qty?,base?,ench?,bonus?,codexId?}, to:{kind:npc\|creature\|faction\|container\|place\|object, ref, name?}, intent?:transfer\|entrust\|gift\|loan\|place\|confiscated\|stolen\|lost, note?}` | declared (DM, when newly revealed loot already belongs to a scene holder) | originates one congruent portable instance directly in `w.itemCustody`, without a fictional pickup or transient active-PC inventory mutation. Uses the same magic overlay/charge/codex builder as `item_changed.add`, stamps current walk segment custody, and refuses duplicate physical instances of one `codexId`. Existing items must use `item_transfer`; PC/corpse destinations are rejected because their native arrays require `item_changed`/`item_transfer`. |
...
| `item_claimed` | `{codexId, by:{kind:pc\|npc\|creature\|faction\|corpse\|bastion\|none, ref?, name?}, lossState:<LEGACY_LOSS_STATES>, at?, factionInterest?, note?}` (aliases `id/item→codexId`) | detected (death/claim/scavenge/`item_changed` fold) or declared (off-screen custody: an NPC sells it to a faction, the DM casts the unknown holder, destruction; the player laying an item in a bastion's vault) | **ITEM-LEGACY §2.1** — the single custody-transition event; writes `r.legacy` on the CODEX item record (`legacyStamp`), never the instance. Refuses `{ok:false,reason:"no-item-record:<id>"}` (missing/non-item), `"bad-loss-state:<x>"` (not in `LEGACY_LOSS_STATES`). A re-declared identical claim → `{ok:true,unchanged:true}` (no ledger line). Hostile/lost transition out of `held` mints a recovery-hook thread; neutral `transferred` never does. Return to a PC's hand resolves it; `destroyed` → claimant none + hook resolved + record persists (legend). **CROWNING-BASTION §7.B1.4 unparked `lossState:"cached"`** (was refused `"bastion-parked"` under ITEM-LEGACY v1, §7.3): with a `w.bastion` on the books it deposits into the vault (claimant becomes `{kind:"bastion"}`, the codexId joins `w.bastion.vault`, no decay/scavenge ever touches it); without one it refuses `{ok:false,reason:"no-bastion"}` |
...
| `walk_advance` | `{toSeg, nodeId?}` (alias `toSegment→toSeg`) | declared (DM, party clears a segment) | WALK-CONSUMPTION (moves the active-walk cursor; `nodeId` defaults to the active walk) |
...
name instances may legitimately coexist with different `conditions`. `item_transfer` adds the
identity-conserving destination step: inventory/corpse native stores or the ordinary
`w.itemCustody` registry, projected to the DM only at the current node. `equip`/`unequip` use **named
slots** (`mainHand`/`offHand`/`armor`), not a single pointer, because two-weapon fighting needs two
weapons equipped at once. All logged to the ledger (kind `inventory`/`inventory-split`/`item-transfer`/
`item-condition`/`equip`) so state is always reconstructable from history. (Capture's confiscation, WALK-CONSUMPTION §6,
