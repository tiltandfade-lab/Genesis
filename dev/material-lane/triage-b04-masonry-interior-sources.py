#!/usr/bin/env python3
"""Measure and render the B04 masonry/interior-floor source gate."""
...
BOARD = PROOF_DIR / "b04-masonry-interior-selected-source-board-v002.png"
RECEIPT = PROOF_DIR / "b04-masonry-interior-selected-source-receipt-v002.json"
SOURCES = (
    ("wall-ashlar-dressed", "Wall / dressed ashlar", "wall-ashlar-dressed-selected-v001.png", "approved B02 sprite-first parent + 512px normalization"),
    ("wall-rough-hewn-block", "Wall / rough-hewn block", "wall-rough-hewn-block-selected-v001.png", "period-contract ImageGen retry v002 + 32px X-only color lock + normalization"),
    ("wall-dry-stack-fieldstone", "Wall / dry-stack fieldstone", "wall-dry-stack-fieldstone-selected-v001.png", "raw ImageGen v001 + normalization"),
    ("wall-brick", "Wall / brick", "wall-brick-selected-v001.png", "period-contract ImageGen retry v002 + normalization"),
    ("wall-plastered-rubble", "Wall / plastered rubble", "wall-plastered-rubble-selected-v001.png", "raw ImageGen v001 + normalization"),
    ("floor-flagstone", "Floor / flagstone", "floor-flagstone-selected-v001.png", "raw ImageGen v001 + normalization"),
    ("floor-plank", "Floor / plank", "floor-plank-selected-v001.png", "raw ImageGen v001 + normalization"),
    ("floor-packed-earth", "Floor / packed earth", "floor-packed-earth-selected-v001.png", "ImageGen v001 + 24px continuous-field lock + normalization"),
    ("floor-cobble", "Floor / cobble", "floor-cobble-selected-v001.png", "raw ImageGen v001 + normalization"),
)
...
def main() -> None:
    ...
    draw.text((36, 26), "B04 MASONRY + INTERIOR FLOORS — SELECTED SOURCE GATE", fill="#f4ead7", font=font)
    draw.text((36, 58), "Walls are front-on; floors are top-down. Single source and locked-aspect 3x3 repeat shown.", fill="#aeb8c8", font=font)
    ...
    for index, item in enumerate(results):
        ...
        draw.text((36, top + 80), f"X {item['metrics']['xBoundaryToInternalP95']:.3f}x p95   Y {item['metrics']['yBoundaryToInternalP95']:.3f}x p95   {item['selectionMethod']}", fill="#ccd3df", font=font)
    ...
    board.save(BOARD)
    payload = {
        "schema": "genesis.b04-material-source-gate.v1",
    ...
    if not args.allow_fail and not all(item["metrics"]["boundaryJumpGate"] for item in results):
        raise SystemExit("B04 source gate failed.")
