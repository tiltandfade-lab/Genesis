...
truth; lore-first purpose provenance; whole-site functional profiles sourced from the DMG/2e DBG
...
distinguishes fast checkpoint commits from final/evening clean close; both reuse the current worktree,
and new worktrees skip LFS smudging unless their task genuinely requires assets.
...
3. **Phase 3 (visual production, GP-2..4) is Fable's to direct** — read
   **`docs/PHASE-3-DIRECTOR-BRIEF.md`** first (the handoff: flip-evidence artifacts, the full Codex
   plugin-rec suite + plan with links, the GP-2..4 terrain, the oracle/spend calls that are Fable's,
   Claude's smallest-correction-first recommendation, and a **§6 OSS-adoption findings** table from a
   3-agent "are we rebuilding the wheel?" pass — `@three.ez/instanced-mesh` for the unbuilt R3 sprite
   instancing (the big one), Open5e `srd-2024` as an SRD-Data cross-check, a free AgX tone-curve, a
   test-PRNG dedup). It rides on the stable, flipped geometry:
