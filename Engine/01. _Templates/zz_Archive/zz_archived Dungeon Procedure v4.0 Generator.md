> [!info] Dungeon Procedure v4.0 Generator
...
>
> v4.0 changes from v3.9: Topology-driven graph construction. Each topology type builds a real connection graph (BFS-ordered, bidirectional). Rooms are output as individual notes and linked in a Canvas file. Exit counts come from the graph — no more topology profile range lookup. Room connections show named targets (→ Room 3 (Hub)). Two-pass generation: area types assigned first, then full content built with cross-references. All 12 topology types implemented with minimum-segment fallback chain.

<%*
// =============================================================================
// DUNGEON SKETCH GENERATOR — Arcana Engine v0.25 · Dungeon Procedure v4.0
