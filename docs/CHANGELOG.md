...
orchestrator re-gating each unit and preserving the continuous Genesis room shell as structural
truth. The operation deliberately admits ten calibrated pilots—not the entire 1,752-asset source
catalog—and never rewrites a rolled walk noun to force a model match.

**Added:** a source-hash-bound Kenney calibration sidecar and schema; normalized donor metadata v2;
six-degree attachment/mount frames with full scene-graph transform baking; a Donor/Socket Workbench
for manual transform, grounding, socket, clearance, footprint, material-family, and QA correction;
an approval-only runtime registry; ten mandatory calibrated pilots; footprint-aware, deterministic,
walk-native visual realization with provenance-preserving fallbacks; and the KGR proving capture set
under `dev/battle-gate/kenney-graphics-repair/`.

**Changed:** unsafe scaled back-face donor outlines retreat to safe presentation; Kenney wall/floor
claims no longer disable the continuous compiled shell; door leaves and frames now cross detach
boundaries in one world frame; realization respects calibrated bounds, yaw, footprint, clearance,
and source hashes; and `docs/KENNEY-GRAPHICS-REPAIR-OPERATION.md` advances `SPECCED` → `BUILT`.

**Fixed:** wrong-axis, oversized/detached, and intersecting pilot placements at the walk-to-graphics
boundary. A live Prison / Asylum bridge playtest found no P0/P1 Kenney alignment or collision defect
at gameplay scale. Its console audit exposed a separate first-render warning: `nearestify()` marked
TextureLoader placeholders dirty before image decode; upload readiness is now guarded by `tex.image`
while nearest filtering still applies immediately.

**Deferred:** automatic admission of the remaining catalog; Kenney structural wall/floor ownership;
P2 Chrome lighting/material readability (overbright flora and dark apertures); and prep-autopilot
service for `prepPending` before a frontier walk can be expected to mount its volumetric spatial plan.
The live roll contained algae, bones, mud, and relief carving, so none of the ten approved furniture
pilots truthfully matched; the renderer correctly kept canonical fallbacks rather than inventing nouns.
