...
// ============================================================================
// 4. Generator retreat -- build/gen-sprite-registry.py --admit-faceted is re-derivable: WITHOUT the
//    flag every entry regenerates "legacy" regardless of the currently-committed state; WITH it, the
//    real 252 candidates come back. Restores the committed (--admit-faceted) state afterward.
// ============================================================================
console.log("\n=== Generator retreat: build/gen-sprite-registry.py --admit-faceted is re-derivable ===");
{
  ...
  check("sanity: the committed registry starts with the real 252 candidate admissions",
    beforeCandidateCount === 252, `got ${beforeCandidateCount}`);
  ...
  check("re-admit: regenerating WITH --admit-faceted restores the real 252 candidate admissions",
    withFlagCandidateCount === 252, `got ${withFlagCandidateCount}`);
  check("re-admit run is byte-identical to the committed --admit-faceted state (deterministic regen)",
    withFlag === before, "regenerated bytes differ from the committed file");
