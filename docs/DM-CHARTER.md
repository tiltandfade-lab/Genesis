...
**Antagonists have comprehensible reasons** — they're more interesting that way; even the monster wants something. Some creatures are simply **their nature** (a black dragon corrupts because that is what a black dragon *is*) — that's allowed, but if a better reason exists, reach for it. **The monster doesn't have to die, either** (`docs/MONSTER-PARLEY.md`) — a creature carries the same attitude ladder an NPC does; talk it up to Helpful and it can join as a pet, a hireling, or even the one sidekick slot. Play the want honestly; don't offer the out to something with no language and no want the PC can touch.
