...
for(const token of ["assetSearch","assetFilter","qaFilter","raw","normalized","genesis","axes","module","ground","wireframe","backface","normals","aabb","footprint","hierarchy","humanScale","6 ft human yardstick","6 ft / 1.2u","orientationIndex","admissionClass","groundOffset","footprintOverrideEnabled","socketList","snapMode","hostSocket","childAsset","exploded","doorSweep","dirty","Reset Asset","Save Asset","Export Snapshot","packPanel"])check(`required UI token: ${token}`,html.includes(token));
...
check("raw diagnostic fallback is an exact two-URI allowlist",js.includes('["textures/colormap.png","Textures/colormap.png"]')&&js.includes('["textures/barrel.png","Textures/barrel.png"]')&&js.includes("if(!entry)return url"));
...
mkdirSync(OUT,{recursive:true});const report={schema:"genesis.kenney-workbench-capture.v1",assetId:existingId,viewUrls:{},materialDiagnostics:{},rawBarrel:{},overlays:{},mate:{},consoleErrors:[]};
...
if(!puppeteer){check("Puppeteer available for required visual gate",false,"puppeteer-core missing");}
  ...
  try{
    ...
    for(const view of ["raw","normalized","genesis"]){await page.click(`[data-view="${view}"]`);await page.waitForFunction(v=>window.__kenneyWorkbench.state.view===v&&document.querySelector("#saveStatus").textContent!="loading " + v + "…",{},view);report.viewUrls[view]=await page.$eval("#loadedUrl",el=>el.textContent);report.materialDiagnostics[view]=await page.$eval("#errors",el=>el.textContent);}
    await page.click('[data-view="raw"]');await page.waitForFunction(()=>window.__kenneyWorkbench.state.view==="raw"&&document.querySelector("#saveStatus").textContent==="raw loaded",{timeout:30000});await page.evaluate(()=>window.__kenneyWorkbench.selectAsset("kenney-retro-fantasy-kit/detail-barrel"));await page.waitForFunction(()=>window.__kenneyWorkbench.state.currentId==="kenney-retro-fantasy-kit/detail-barrel"&&window.__kenneyWorkbench.state.view==="raw"&&document.querySelector("#saveStatus").textContent==="raw loaded",{timeout:30000});report.rawBarrel=await page.evaluate(()=>{const state=window.__kenneyWorkbench.state;let meshCount=0,vertexCount=0;state.model?.traverse(object=>{if(!object.isMesh)return;meshCount++;vertexCount+=object.geometry?.attributes?.position?.count||0;});return{assetId:state.currentId,url:state.modelUrl,modelName:state.model?.name,meshCount,vertexCount,diagnostic:document.querySelector("#errors").textContent};});
    await page.evaluate(id=>window.__kenneyWorkbench.selectAsset(id),existingId);report.materialDiagnostic=await page.$eval("#errors",el=>el.textContent);
    report.overlays=await page.evaluate(()=>{const api=window.__kenneyWorkbench,marker=api.scene.getObjectByName("overlay-human-yardstick"),metrics=api.humanYardstickMetrics(),before=marker.matrixWorld.elements.slice(),model=api.state.model,orientation=api.scene.getObjectByName("orientation-quarter-turn"),savedPosition=model.position.toArray(),savedRotation=orientation.rotation.y;model.position.set(savedPosition[0]+7,savedPosition[1]+3,savedPosition[2]-5);orientation.rotation.y+=Math.PI/2;api.scene.updateMatrixWorld(true);const after=marker.matrixWorld.elements.slice();model.position.fromArray(savedPosition);orientation.rotation.y=savedRotation;api.scene.updateMatrixWorld(true);return{axes:api.scene.getObjectByName("overlay-axes")?.visible,ground:api.scene.getObjectByName("overlay-ground-plane")?.visible,module:api.scene.getObjectByName("overlay-module-grid")?.visible,sockets:api.scene.getObjectByName("socket-overlays")?.children.length,humanScale:{...metrics,legend:document.querySelector("#humanScaleLegend").textContent,legendVisible:!document.querySelector("#humanScaleLegend").hidden,independent:before.every((value,index)=>Math.abs(value-after[index])<1e-12)},canvas:{width:document.querySelector("#viewCanvas").clientWidth,height:document.querySelector("#viewCanvas").clientHeight}}});report.rotatedBounds=await page.evaluate(()=>window.__kenneyWorkbench.rotatedBoundsControl());
    await page.click('[data-overlay="humanScale"]');report.overlays.humanScale.toggleOffHidden=await page.evaluate(()=>!window.__kenneyWorkbench.scene.getObjectByName("overlay-human-yardstick").visible&&document.querySelector("#humanScaleLegend").hidden);await page.click('[data-overlay="humanScale"]');report.overlays.humanScale.captureVisible=await page.evaluate(()=>window.__kenneyWorkbench.scene.getObjectByName("overlay-human-yardstick").visible&&!document.querySelector("#humanScaleLegend").hidden);
    ...
    check("real GLTFLoader loads raw retro-fantasy barrel geometry",report.rawBarrel.assetId==="kenney-retro-fantasy-kit/detail-barrel"&&report.rawBarrel.url.endsWith("/kenney-retro-fantasy-kit/detail-barrel.glb")&&report.rawBarrel.modelName?.startsWith("raw:")&&report.rawBarrel.meshCount>0&&report.rawBarrel.vertexCount>0,JSON.stringify(report.rawBarrel));
    check("raw barrel names only its actual absent texture",report.rawBarrel.diagnostic.startsWith("RAW MATERIAL UNAVAILABLE: Textures/barrel.png is absent")&&!report.rawBarrel.diagnostic.includes("Textures/colormap.png"),report.rawBarrel.diagnostic);
    ...
    check("human yardstick measures exactly 1.2u in world space",Math.abs(report.overlays.humanScale.height-1.2)<1e-6,JSON.stringify(report.overlays.humanScale));
    check("human yardstick stands on calibrated-root ground y=0",Math.abs(report.overlays.humanScale.groundY)<1e-9&&report.overlays.humanScale.parent==="workbench-calibrated-root",JSON.stringify(report.overlays.humanScale));
    check("human yardstick is independent of asset TRS and orientation",report.overlays.humanScale.independent,JSON.stringify(report.overlays.humanScale));
    check("human yardstick toggle and 6 ft / 1.2u capture legend are visible",report.overlays.humanScale.visible&&report.overlays.humanScale.onScreen&&report.overlays.humanScale.legend==="6 ft / 1.2u"&&report.overlays.humanScale.legendVisible&&report.overlays.humanScale.toggleOffHidden&&report.overlays.humanScale.captureVisible,JSON.stringify(report.overlays.humanScale));
    check("absent raw colormap fallback is explicit in capture diagnostics",report.materialDiagnostic.includes("RAW MATERIAL UNAVAILABLE")&&report.materialDiagnostic.includes("Textures/colormap.png")&&!report.materialDiagnostic.includes("Textures/barrel.png"),report.materialDiagnostic);
    check("material diagnostics reset instead of accumulating across views",report.materialDiagnostics.raw.includes("Textures/colormap.png")&&report.materialDiagnostics.normalized===""&&report.materialDiagnostics.genesis==="",JSON.stringify(report.materialDiagnostics));
