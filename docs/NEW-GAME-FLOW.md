...
2. ☑ **Chat-first interface shell** (§9) — **DONE 2026-06-21 (Claude Code).** The World view is now the **DM conversation, centered**: a scene header (place + diegetic clock) + the DM feed + action input, with a collapsed "⚙ World & transitions" disclosure holding the explore/time controls. The world's panels live in a **granular left icon rail** (Story · Character · Map · Ledger · Gazetteer · Powers, + Universe/Oracle), each **revealing per the Curve of Revelation**, and **slide into a column beside the chat** when opened (the Disco-Elysium two-pane). The character sheet is now a side panel (`renderCharacterPanel`). **Waking cinematic:** the bardo fades to black and dissolves into the chat-first view (`wakeIntoWorld`); if the DM bridge is live, `autoOpenScene` fires the opening turn so the player wakes into the DM's first words. (`render.js`/`play.js`/`chrome.js`/`sheet.js`/`bardo.js`/`state.js`; `.wrap.ingame` + `#wakeFade`.) *v1 shell (2026-06-19) — tabs → left rail, immersive creation passages — superseded by this.*
