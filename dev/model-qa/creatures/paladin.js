/* dev/model-qa/creatures/paladin.js — the full-plate holy knight landmark table (whole-object probe).
   ...
   primitives, every vertex in one model frame, no anchors. The TOWER SHIELD is authored first
   (left arm derives to its back face) and the WARHAMMER second (right fist derives to its grip) —
   held items always precede the limbs that hold them. Silhouette-first: a tall crested/plumed
   closed helm, big flared pauldrons, a large kite/tower shield, a warhammer held upright, a
   tabard front over the cuirass, and an upright square heavier mass than the fighter — full-metal
   plate read that must never be confused with the fighter's cloth tunic or the barbarian's bare
   chest. Runs at the base humanoid height (~1.48 head-top) but reads heavier/blockier throughout. */
