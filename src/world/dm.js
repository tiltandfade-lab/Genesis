...
function applyEvent(w,e){
  ...
  switch(e.type){
    ...
    case "rest":{
      ...
      // TRANSITION-CONTRACT.md §3.8 — a rest ticks the clock BEFORE recovery (+480 long, +60 short).
      // The UI passTime path never emits `rest` (it calls restRecover directly, play.js) — no double tick.
      ...
      addLedger(w,"outcome",{kind:"rest",pc:t.c.name,rest:kind,restored:summary,recharged:recharged,
        exhaustion:exhaustionAfter,advanceMin:restMin,source:src},
      ...
      return {ok:true,rest:kind,restored:summary,exhaustion:exhaustionAfter,minutes:restMin};
    }
