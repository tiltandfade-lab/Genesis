/* dev/model-qa/creatures/prop-pool.js — the STAGNANT POOL: a dungeon floor SET PIECE (whole-object
   ...
   decal. Sits on a widened base disc (r=0.62, matches registry discR). A LOW piece — the water
   surface is near floor level. RESIZED 2026-07-08 (prop-scale-contract): basin widened to ~1.2u dia
   (staged ~5 ft) — the old 0.8u basin staged at 3.4 ft, a puddle, not a pool.
...
export function buildPool(){
  ...
  const rOut = 0.60, rIn = 0.48;               // rim outer/inner radius
     ...
     it reads as still liquid, not a stone floor patch. ===== */
  {
    ...
    quad(V(-0.16,surfY+0.002,0.09),V(0.03,surfY+0.002,0.16),V(0.06,surfY+0.002,0.03),V(-0.13,surfY+0.002,-0.03), P.glint, 0.04);
  }
     ...
     stagnant tell). Low flat blobs a hair above the water. ===== */
  {
    ...
    quad(V(-0.35,y,-0.03),V(-0.13,y,-0.16),V(-0.03,y,0.09),V(-0.25,y,0.19), P.algae, 0.07);
    quad(V(0.09,y,0.22),V(0.31,y,0.13),V(0.38,y,-0.06),V(0.16,y,-0.03), P.algaeLt, 0.06);
    quad(V(0.03,y,-0.31),V(0.25,y,-0.34),V(0.28,y,-0.16),V(0.06,y,-0.16), P.algaeDk, 0.06);
  ...
  /* base disc — widened with the basin (r=0.62, matches registry discR; the rim seats on it). */
  {
