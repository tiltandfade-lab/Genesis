...
| Party/social/inventory minimum | One player-owned main PC; allies/sidekicks remain canonically present through a party anchor and coarse set formation outside combat, deploy within an approach-bounded legal zone, and then act independently; one persistent dungeon relationship and exact consequential item custody; narration alone cannot detach a companion or change location/custody | C1C/C1D/C2D | C2D/C4A/C4B plus later Wave 6/7 behavior gate | Independent companion noncombat activity, richer formations/commands, split-party, richer social places, broad inventory ecology; Waves 5-7/9 | MVP cases cannot represent intended party/social/economic choice, or autonomous allies feel arbitrary/unusable | MAPPED IN PART; pre-alpha control premise accepted, exact deployment control and ally behavior open |
...
- P10.9 F10.9a accepted the transactional SceneLineage handoff and added C2D as the first retained continuity proof;
  ...
  mapped only in part. F10.9c now assumes one player-owned main PC, formation-bound companions outside combat, and
  independent allied combat action, but the player's exact deployment authority remains open. Generated P10.9
  follow-ups still own ambiguous placement conflict, split-party, travel, town, CrisisChain, and broader recovery/
  visual-memory obligations before its MVP gate can be complete. Waves 6-7 still own exact allied behavior.
