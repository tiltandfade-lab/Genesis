...
**Date:** 2026-07-30 · **Type:** study-capture · **Status:** rulings banked here, canon ports pending (see bottom) · **Owner:** Adam; full material lives in the `anti-default` project (`~/Desktop/Work/projects/anti-default`)

## What happened

Adam identified that AI-designed interfaces converge — "you can spot an AI-designed UI immediately" — and that the convergence is in **patterns, not selections**. The proof came from this repo: genesis.html and a stranger's game (Dungeons & Dynasties, bretticus82.itch.io) request `Cinzel:wght@500;600;700` character-for-character identically, both name EB Garamond, and both land on the same antique gold `#c9a24b` to the digit. Two studios that never met, one room. The lesson: there is no single "AI mode" — every one-line brief has its own attractor, and escaping one attractor's values just lands you on the neighboring attractor. Full evidence: `anti-default/census/2026-07-30-genesis-vs-dnd.md`.

A dedicated studio project, **`anti-default`**, now holds the escape machinery: a twelve-exhibit close-read corpus (Book of Kells → The Green Knight), a ten-law pattern library, a banned-tells ledger built from Adam's verdicts (14 tells as of 2026-08-06), an approved-registers catalog, the portable `$anti-default` agent skill, project contract/audit templates, and a census tool (`anti-default/tools/design-census.sh`) that catches convergence across interface source trees. Three rounds of taste cards proved the loop works: by round three Adam ruled "nothing is reading too AI in this set." The project's former abstract-noun name was itself killed as tell T14 on 2026-08-06.
...
2. **The letterpress broadside register is approved for in-world print ephemera** — job boards, wanted posters, news artifacts, posted notices. Spec: black ink dominant, mixed faces line by line, size jumps set by line length, second ink rationed to the one working word. See `anti-default/laws/APPROVED-REGISTERS.md` R1.
...
5. **Renders/mocks:** `anti-default/cards/turn-order-c-two-states.pdf` (current), plus the exploration decks alongside it.
...
- Project: `~/Desktop/Work/projects/anti-default/` — `README.md` (charter + thesis), `skill/anti-default/` (portable agent skill), `laws/PATTERN-LIBRARY.md` (ten laws), `laws/TELLS.md` (banned tells, T1–T14), `laws/APPROVED-REGISTERS.md` (approved voices + kills), `corpus/fantasy-typography-field-guide.pdf` (the twelve-exhibit study), `cards/*.pdf` (taste cards and explorations), `contracts/PROJECT-CONTRACT.md` (standalone contract), and `tools/design-census.sh` (the convergence census).
