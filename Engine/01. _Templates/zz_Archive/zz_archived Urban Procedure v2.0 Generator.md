> [!info] Urban Procedure v2.0 Generator
...
>
> v2.0: All 16 topologies. Tier selection (T1/T2) with Threat Identity creature pools. Three finale tracks (Combat, Social, Discovery). Topology-aware segment fields. Topology-native canvas layouts. Full v1.5 table suite.

<%*
// =============================================================================
// URBAN SKETCH GENERATOR — Arcana Engine v0.25 · Urban Procedure v2.0
// v2.0: All 16 topologies. T1/T2 tier with Threat Identity creature pools.
//       Three finale tracks (Combat, Social, Discovery). Topology-aware
//       segment fields. Topology-native canvas layouts. Full v1.5 tables.
// =============================================================================
...
async function generateSegmentNote(segNum, node, exits, segNumbers, nodeMap, topoName, threatCtx, adventureName, tier) {
...
for (const nodeId of bfsOrder) {
  ...
  if (node.isFinale) {
    ...
    noteContent = await generateSegmentNote(segNum, node, exits, segNumbers, nodeMap, resolvedTopo, threatCtx, adventureName, tier);
  }
...
const briefing = `In the ${skinName.toLowerCase()} ${typeArch.toLowerCase()} shaped by ${originCat.toLowerCase()}, ${catalystResult.toLowerCase()}. The local threat is ${threatCtx.id.toLowerCase()} — contact/composition varies by tier. The cultural motif of ${motifName.toLowerCase()} (${motifDesc.toLowerCase()}) is ${modName.toLowerCase()} — ${modDesc.toLowerCase()}.`;
...
*Generated with Arcana Engine v0.25 — Urban Procedure v2.0*
`;
