/* dev/model-qa/creatures/halforc-druid.js — RACE×CLASS bespoke starter (POLISH-WAVE-1 §F4).
   The FIXED half-orc race (heavy/broad frame, gray-green skin, heavy jaw + tusk nubs + jutting brow)
   wearing the DRUID kit (druid.js signature: a GNARLED forked STAFF authored first + raked to a
   leaned-on communing lean, a hide/leaf MANTLE over the shoulders, an ANTLER/branch headdress, a
   ratty robe with a bone-charm belt). The half-orc's mass reads as a big weathered wilderness elder;
   the tusks + brow keep the race read under the antler rack. One whole-object function, no anchors. */
import { THREE, V, quad, tube, stack, ring, stitch, capFan, blob } from '../probe-lib.js';

export function buildHalfOrcDruid(){
  const P = {
    ...
    cowl:0x453824, cowlDk:0x2f2618, eye:0x140f0c,
    cord:0x6b5535, disc:0x453a2c, discTop:0x554836,
  };

  /* half-orc landmarks, but the robe is the lower body (hem at the disc) */
  const L = {
    hemY:0.06, kneeY:0.42, waistY:0.815, chestY:1.045, shldY:1.130, neckY:1.180,
  ...
  /* ROBE — ratty under-layer, hips->neck (half-orc broad) */
  stack([
  ...
  /* belt + bone charms */
  stack([
    {y:0.790, rx:0.212, rz:0.168, hex:P.cord},
    {y:0.830, rx:0.209, rz:0.165, hex:P.cord},
  ], 8, {});
  {
    blob(0.175, 0.72, 0.135, 0.052, 0.058, 0.040, P.hideDk, 6, 4);
    for(const [ox,oz] of [[-0.12,0.155],[-0.07,0.170],[-0.02,0.160]]){
      const top=V(ox,0.790,oz), bot=V(ox+0.005,0.690,oz+0.01);
  ...
  /* HIDE/LEAF MANTLE — over the shoulders, uneven points (half-orc broad) */
  {
    const n=8, ph=Math.PI/n;
    const mantleBands=[
      {y:0.985, rx:0.310, rz:0.230, hex:P.hideDk},
      {y:1.08,  rx:0.282, rz:0.205, hex:P.hide},
      {y:1.15,  rx:0.250, rz:0.175, hex:P.hideLt},
    ];
    ...
    for(const [i,yy,zz] of [[1,1.09,0.18],[4,1.10,-0.16],[6,1.03,0.12]]){
  ...
  /* HIDE COWL — low, open-faced, under the antlers */
  {
    ...
    const bands=[
      {y:L.neckY-0.004, rx:0.152, rz:0.142, hex:P.cowlDk},
      {y:L.jawY+0.010,  rx:0.176, rz:0.162, hex:P.cowl},
      {y:L.browY-0.01,  rx:0.180, rz:0.164, hex:P.cowl},
      {y:L.crownY+0.01, rx:0.140, rz:0.128, hex:P.cowl},
    ];
  ...
  {
    for(const s of [-1,1]){
      const root = V(s*0.058, L.crownY+0.03, -0.01);
      const mainTip = root.clone().add(V(s*0.055, 0.200, -0.030));
      tube(root, mainTip, 0.018,0.011,6,P.antlerDk);
      const fork1Base = root.clone().lerp(mainTip, 0.38);
      const fork1Tip = fork1Base.clone().add(V(s*0.095, 0.080, 0.060));
      tube(fork1Base, fork1Tip, 0.011,0.005,5,P.antler,{capB:{hex:P.antler}});
      const fork2Base = root.clone().lerp(mainTip, 0.68);
      const fork2Tip = fork2Base.clone().add(V(s*0.078, 0.100, -0.050));
  ...
  /* LEGS — visible below the ragged robe hem, worn wrap-boots (half-orc broad) */
  {
    for(const s of [-1,1]){
