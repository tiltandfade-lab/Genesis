...
The first pipeline pass for Golden Site 12. Before today the catalog carried Site 12 in one
row of the vernacular matrix (*"exotic · smallest structural buy · organic/impossible
connectors; biggest skin buy"*) and one line of the ontology contract (*"substrate/ownership
stress case — non-building identity, unusual extent, motion or living ownership"*). It has
no thin brief and no prior research.
...
|---|---|---|---|---|---|
| 12 Anomalous / Living / Mobile | **PARTIAL** | **OPEN** | **PARTIAL** | **OPEN** | depth law's three lanes served (sourced breadth sweep, 13-image licensed reference lane, 8-map FFT cohort) and demand re-measured against the live engine; but no measured drawing was obtained, no culture/occupant study exists, family F has no evidence and cannot get any, every build order below is PROPOSED, and no fixture has rendered |
...
**None.** No founder ruling exists for Site 12. Every recommendation in this document is
`PROPOSED` or `DEFAULT` under the catalog's recommendation classes, and nothing here is
...
7. the reading of the existing footing tables as Site 12's load-band and trusted-path
   ...
   another lane; and
8. all five founder questions in §15.5, which arrive as proposed build orders under the
   generator principle, never as A-or-B choices.
...
- **DEFAULT — the site is mostly quiet.** Living and unreliable substrates are sparsely
  ...
  (`S12-A2`). Density is the exception, and the ontology's §4.3 honest-empty-space rule
  applies with unusual force.
- **DEFAULT — the record of former occupancy outnumbers the occupants.** Old anchor scars,
  abandoned stake fields, previous waterlines, healed cuts, the last island's sunken mat.
- **LICENSED — intention.** The substrate may want, remember, or bargain only when world
  facts license it (a rolled Becoming, a rolled Buried Power, a `place-master-setting`
  Strange+ row). It is never an automatic flavour of unusual ground.
...
Per the guidelines' step 3 — five related expressions sharing one relational grammar.
...
> **`SUB-RAFT-01` — a moored raft-quarter on a rising river.**
>
> A working floating settlement at the moment the river is coming up: half a dozen pontoon
> decks and one raft-mill, lashed into a cluster and tethered to a field of driven stakes
> and one masonry pier; plank spines between the decks; one gangway to the bank; the mill
> wheel turning in the gap between two hulls. One raft has already parted its mooring and
> fetched up hard against a piling downstream, and the crew is deciding whether to cut it
> loose or warp it back.

**Why this and not `The Dorsal Market`.** The Dorsal Market is the site's most exciting
expression, it is a real rolled TIYL start, and it is Strange-band canon
(`place-master-setting` 90). It is also the wrong first build: a walkable gargantuan animal
is the entire Ideal asset library in one object, and choosing it would let the site's
...
2. **The trusted path** — the reliable spine over unreliable ground. Continuous, legible,
   and narrower than the site.
3. **The anchor field** — the stakes, moorings, piles, lashings, or growing joints that
   hold the site where it is. Inspectable, damageable, and contested.
4. **The knot** — the one dense area where the site's activity actually is: the market
...
**Optional-but-licensed:** *the underside* (the piling forest, the hull's below-deck, the
space between the mat and the water) — a second, tight, intentionally simple interior which
appears whenever the datum permits.

### 4.2 The vertical quantum, restated as construction

The engine's established vertical quantum is **h = 2.5 ft (0.76 m)** (Site 5 spec §"Working
spatial specification"). Restating the site's levels as real construction before proposing
any number:

- *A plank deck on a barrel or log pontoon.* The pontoon floats with most of its bulk
  submerged; a walking deck sits a short step above the waterline so it stays dry in chop.
  Tonlé Sap pontoons are barrels or bamboo bundles under a plank platform. **≈ 1h.**
- *A moored hull's deck.* The Ráckeve boat mill's deck sits roughly a metre above the
  water, with the wheel hung in the gap between two hulls (`S12-B1`, `S12-B2`).
  **≈ 1h, occasionally 2h for a laden hull.**
- *A wheeled cart deck.* The deck height is the wheel radius; a *ger tereg*'s wheels are
  large enough that the deck is chest-high on a standing adult (`S12-B3`). **≈ 2h.**
- *A stilt house at flood datum.* Kampong Phluk's pilings run through roughly two storeys
  of exposed timber at low water (`S12-A3`). **The real swing is up to 10 m (13h) —
  explicitly too large for a tactical map**, because a 13h swing means the high and low
  states share no geometry at all.
- *A mast or fast point.* A mill roof-ridge or a small mast stands a few metres above its
  deck. **≈ 3h–5h.**

### 4.3 Provisional grid hypotheses — starting points for clay, not architecture

| element | hypothesis | construction basis |
|---|---|---|
| main deck / knot floor | **2 cells (10 ft) clear** wherever goods, carts, or the presentation-large witness must pass | mill deck and market arcade working width |
| trusted path | **1 cell (5 ft)**, and it is the site's narrowest legal route | real boardwalks and catwalks are 0.6–1.2 m — *narrower* than one cell; 1 cell is already generous |
| deck above water plane | **1h** (raft, hull); **2h** (wheeled deck) | §4.2 |
| cyclic swing (high state ↔ low state) | **2h–4h (5–10 ft)** | Tonlé Sap's 10 m is the real upper bound and is rejected for tactical maps; 2h–4h keeps ids shared between states |
| fast point above normal floor | **3h–5h** | mast / roof-ridge / dorsal ridge |
| anchor spacing | **2–4 cells** between stakes/piles in a holding field | Uros islands are staked at multiple points around a ~50 ft × 50 ft mat; a 2–4 cell rhythm gives a mat of that size 4–8 anchors |
| substrate thickness (mat/deck) | **1h nominal**, i.e. what is under the floor begins one quantum down | Uros root block 1–2 m; quaking mat 0.5–several m; both bracket ≈ 1h |
| perimeter halo (reactive ground) | **1–2 cells** of dead/marked transition outside the anomaly | Centralia's vent patch has a dead-tree ring with healthy scrub to the line (`S12-D3`) |
...
in a 10-ft square") that the live roller currently discards (§8.2a). The bands below are a
proposed *reading* of that existing content, not a new system:
...
recovery**, which is what makes weight a plan rather than a trap.
...
From `BATTLEMAP-TOWNTRAY-COMPOSITION.md` §10.3's twelve structural coverage dimensions,
Site 12 banks:

- **narrow bridge / crossing** — the trusted path; the site's strongest and most
  transferable contribution;
- **basin and rim** — the crater/maw posture (FFT 114 END read); a rim of ground around a
  central void with forced circumferential circulation;
...
Six of twelve from one site. **Two of the six — basin-and-rim and ravine/water-channel — are
achieved by *subtracting* from terrain the engine already builds**, and a third (the
plate-and-gap archipelago, which §10.3 does not name) is the same operation. This is the
concrete rebuttal of the "biggest skin buy" assumption.
...
|---|---|---|---|
| **trusted path** | speed, certain footing, legibility, a route a plan can be built on | it is watched, it is narrow, it is the obvious approach, and it can be cut in one place | ordinary and small standees; **not** presentation-large without a stated exception |
| **costed ground** (mat, shallows, crust, flank) | flank, surprise, and reach to places the path never goes | slower, noisier, load-banded, state-dependent, and sometimes one-way | creature- and state-dependent; large creatures are often *worse* off here |
| **the underside** (pilings, below-deck, mat-under) | concealment, access to the anchor field, a route beneath a controlled deck | crouched or small envelopes, near-total darkness, and no retreat upward except at known hatches | small and ordinary; explicitly capacity-limited |
| **the water / void itself** | the only route to a detached plate or a parted raft | swim/climb cost, gear loss, the clock, and possible one-way commitment | state-gated; illegal for some creatures by fact, not by fiat |

Changing a seed may change which route is strongest. It may **not** erase the choice, and
no route may lead on speed, safety, capacity, and information at once.
...
2. **Anchorage** — stakes, moorings, piles, lashings, growing joints. Has a count, a
   condition, a holding direction, and a recovery (re-drive, re-lash, re-warp).
3. **The clock** — tide, season, drift, growth, rot, spread, sounding. Has a phase, a
...
### 6.2 Factorized state — no single `substrateState` switch

The DM and UI may show friendly labels ("the mat is going", "she's sounding", "the crust is
thin here"). Those labels are **derived**, never stored. Stored facts:

- **integrity:** sound / strained / critical / failed — *per surface region*, not site-wide;
- **hold:** secure / slipping / parted / re-set — *per anchor*;
- **phase:** the clock's current position, its period, and its next event time;
- **datum:** the current level, and which committed ids are above and below it;
...
- **Add to or breach the substrate** — lay fresh reed/plank over a weak patch to raise its
  ...
  crust is a *layer*).
- **Change the datum** — open the sluice, wait the tide, cut the flotation, let the mat
  take water. Both states are the same map; the ids persist.
- **Extend, cut, or re-lay the trusted path** — a plank is a portable route. So is its
  absence.
- **Pull the gangway** — the mobile site's portal is a prop; removing it makes the site an
  island and traps everyone on both sides of the decision.
- **Pay, defer, or refuse the tithe** — the tenancy circuit is a full social lever with
  spatial consequences (which decks you may cross, which anchors you may touch).
- **Provoke or settle the host** — noise, blood, fire, weight, or ritual. High cost, and
  the recovery is rarely symmetrical.
- **Take, hold, or drop the fast point** — sightline, rally, and the site's own integrity in
  one object.

### 7.2 Three demonstrable plans on `SUB-RAFT-01` (one non-combat)

1. **Along the spine (direct).** Take the gangway and the plank spines straight to the
   knot. Fast, certain, fully observed, and one cut plank strands you. Best when the clock
   is against you.
2. **Under the decks (stealth / non-combat).** Enter the piling forest at low state, move
   beneath the occupied decks to the anchor field, and re-warp the parted raft back into
   the cluster without ever meeting the crew — resolving the site's own problem as the
   objective. Crouched envelopes, darkness, no upward retreat except at hatches.
3. **Change the datum (systemic).** Let the river do the work: cut two moorings on the
   upstream side and let the rising stage swing the cluster, opening a gap that did not
   exist and closing one that did. Costs time, is publicly attributable, and rearranges
   the map for everyone including you.

**Reject the first build if:** one route always wins; the anchors are scenery; the low and
high states are two unrelated maps; the quiet field gets carpeted in props to prove it was
used; a large creature has no meaningful tradeoff; retreat is a replay of the ingress; or a
changed seed keeps the picture by destroying the choice.

---

## 8. Current roller sources, status, retained compositions, receipt

Statuses use `GOLDEN-SITE-ROLLER-PRESERVATION-LEDGER.md`'s vocabulary. **This lane did not
edit the ledger**; the records below are proposed for it. Full evidence:
`Reference/Anomalous-Living-Mobile-Study-0727/engine-evidence.md`.

### 8.1 Sources

| source | id / path | consumer verified | status | what it gives Site 12 |
|---|---|---|---|---|
| Master Setting | `place-master-setting` | `src/creator/bardo.js:14` (hometown beat, `ht_setting`); `src/engine/codex-roll.js:660` | **`LIVE`** | 15 of 100 substrate-anomaly settlements across all five bands (4 Grounded) |
| Starting-state pressures | `starting-state-pressures` / `SS.pInternal`, `SS.pExternal`, `SS.pImpersonal` | `src/engine/world-gen.js:29` `rollStartingState()` | **`LIVE`** | the transform door; two pressures rolled at every world genesis |
| The Becoming | `SS.cBecoming` (d8) | `src/engine/tables.js:12–18` `concretize()` via `SS_CONC` | **`LIVE`** | the substrate-rewrite generator; flat d8 |
| The Intrusion | `SS.cIntrusion` (d6) + `SS.cIntrusionManifest` (d4) | same | **`LIVE`** | foreign law imposed on ground, with a manifestation shape |
| The Buried Power | `SS.cBuried` (d8) | same | **`LIVE`** | rows 1, 2, 8 are substrate cases (sleeper beneath · structure surfacing · land reverting) |
| Dungeon type | `dungeon-type` | `src/engine/dungeon-walk.js:613` | **`LIVE`** | rows 98–100: Megastructure ×2, Living Hive ×1 |
| Dungeon area / elevation / feature | (as consumed by `dungeon-walk.js`) | — | **`LIVE`** | room shapes and elevation the substrate skins can ride |
| Wilderness feature | `wilderness-feature` | wilderness walk arrival | **`LIVE-COMPOSED`** | the 255-value curiosity table contains many single-object substrate oddities (floating rock steps, whirlpool/siphon, singing crystals) — reachable, not composed into sites |
...
This was found by reading the table source rather than assuming a gap, and it is the single
biggest correction to §13's invent list.

**`wilderness-footing` (d200, Spark class) already contains the load-band relation.** Its
`Thin Ice / Fragile Crust` rows state a rolled load limit outright — *"Breaks if weight
exceeds 3d10×10 lbs in a 10-ft square; fall through"* — appearing at four different coverage
geometries (`One 20x20 patch`, `1d4 10x10 patches`, `25% of Area`, `Outer Perimeter`). Its
`Deep Bog / Thick Tar` and `Foul Bog / Stagnant Peat` rows author costed-but-passable
ground, again across several coverage geometries.

The table is **live** — `wild-walk.js:182` rolls it — but the call is
`walkPick("wilderness-footing", 1)`, which takes **column 1 only (Surface Flavor)**.
**Columns 2 (Coverage Area) and 3 (Mechanical Impact & Tracking) are discarded at the
consumption boundary.** The rolled load limit, the patch geometry, and the tracking DC never
leave the table.

**`urban-footing` (d100, Spark class) already contains the trusted-path failure grammar** —
`Sturdy Boardwalk` (100% / 5-ft path), `Rotting Boardwalk` (collapses under bludgeoning or
area damage), `Missing Planks` (scattered 5×5 gaps, DC 12 to cross at speed),
`Makeshift Plank` (a 5-ft bridge over a chasm), `Flooded Street`, `Shallow Water`,
`Deep Mud`. **No code path calls it.** Its only consumer is a procedure markdown, which
makes it `ORACLE-MANUAL` under the ledger's vocabulary — usable by a human at the table,
unreachable from the urban walk.

Consequences for this spec, all `PROPOSED`:

1. **Site 12's `TARGET-ADAPTER` gets smaller and more honest.** The adapter's job is to
   *read columns 2 and 3* and to route `urban-footing`, not to invent a load system.
2. **The coverage-geometry column is a semantic gift.** "One 20×20 patch" / "Outer
   Perimeter" / "10-foot wide strip" is exactly the perimeter-and-patch vocabulary §2.3
   requires of a Site-12 anomaly, already authored 200 rows deep.
3. **The preservation risk is real and specific.** If a Site-12 adapter introduces its own
   load model, the engine will end up with two — one live-but-truncated, one new. The
   ledger's governing rule applies: preserve the factorized ability, do not build a
   parallel path.
4. **This is a spawn-audit finding first.** It is recorded in
...
stated original purpose is **"Anomaly / Ancient technology,"** and both atmosphere strings
...
- **`RC-SUBSTRATE-01` — the tidal settlement.** `place-master-setting` 15 *The Drowned Port*
  ("half the streets only accessible during low tide", **Grounded**) — the single
  most-rolled world-origin in the TIYL batch (3 of 12). Retain as the proof that a Site-12
  site can be entirely mundane. Its two datums must survive as one map.
- **`RC-SUBSTRATE-02` — the living mobile market.** `place-master-setting` 90 *The Dorsal
  Market* (**Strange**), as rolled in TIYL Start 3, complete with its rolled entry triplet
  (*carrying something to deliver* / *a patron's eye already on you* / *a stranger* to
  **The Tithe-Keepers**) and its rolled tension (*a guild or cartel is tightening its grip
  on a vital trade*). The same roll paired a living mobile market with a dominant power
  literally named for collecting a due — and `intel/tiyl-starts.md` §3.1 reads `bornWhere`
  as the opening-scene anchor, which puts the two in the same scene. **Suggestive, not
  proof**: the report does not settle whether the faction attaches to the whale-market
  itself or to the arrival town. Retained because the tenancy vocabulary showed up on its
  own, which is the cheapest evidence available that FQ-3 is pulling on something real.
- **`RC-SUBSTRATE-03` — the reactive crust settlement.** `place-master-setting` 87 *The
  Shimmering Maw* (**Strange**) as played across the 2026-07-07 marathon: a settlement
  suspended over a fused-glass crater that the transcripts treat as owed, called-to, and
  taking. Retain the *relations*, not the prose.
- **`RC-SUBSTRATE-04` — the Becoming as substrate.** `SS.cBecoming` row 1 — "a single
  substance — glass, stone, metal, salt, fungus, or silk — creeping from a source" — reached
  through `pInternal` 20 (Volatile). Retain as the transform-door acceptance case; it is
  also, mechanically, how a Shimmering Maw comes to exist.

Preserve the **factorized ability** to produce these, never a seed-specific branch.

### 8.4 Receipt

`rollerLineage` must retain: world master/arch/nearby/faction/pressure axes · the
`place-master-setting` or `dungeon-type` row that named the place · every pressure with its
source tag, concretize tag, and concrete row · the walk family and arrival direction · local
area/elevation/feature refs.

`sourceRollRefs` must retain the exact roll refs for: the settlement/dungeon row, both
pressure rolls, any concretize roll and its manifestation roll, and the world-context roll.
...
6. Lay the **trusted path** from landing to knot. Narrower than the site. Continuous in the
   high state; possibly discontinuous in the low.
7. Place the **anchor field** — position, hold direction, and what each anchor holds. This
   must happen before the knot, because the anchors decide what can be where.
8. Place the **knot**, small relative to the footprint, and let the host program's
   operating model own its interior roles.
9. Place the **fast point** on the most reliable anchor.
10. Declare the **quiet field**. Do not populate it.
11. Realize the **underside** if the datum licenses it.
12. Apply **host state** to specific circuits and propagate only causal consequences.
13. Reserve approaches, objectives, retreat, cover, quiet ground, and interaction space.
14. Validate fixed camera, routes, standees, collision, load bands, both datum states, and
    light ownership.
15. Dress retained ids. Never move gameplay geometry to improve a beauty shot.

---

## 10. Rejection rules and deterministic fallback
...
If placement fails, relax in this order and record the relaxation:

1. move non-canonical dressing;
2. widen or re-route the trusted path within its envelope (never past 2 cells);
3. reduce the knot's building count while keeping every host-program role;
4. reduce the anchor count to the minimum that still holds the site (never below 2, and
...
- **tenancy form** — rent · tithe · tribute · labour-share · guest-right · squat · claim by
  maintenance · claim by descent · claim by having survived it;
- **who or what is the counterparty** — a keeper caste, a family, a guild, the host itself,
  the dead, nobody (and the fiction that nobody is a party);
- **anchor practice** — how anchors are made, marked, inherited, moved, and what cutting one
  means socially;
- **maintenance ritual** — who adds reed, re-drives stakes, re-lashes, re-warps, and on what
  cadence (the Uros two-week cycle is a *social* institution before it is a repair job);
- **the clock's ceremony** — what is done at the turn of the tide, the sounding, the freeze,
  the flowering;
- **the trusted path's law** — who may lay a plank, who may lift one, who yields;
- **the margin's law** — what happens at the edge; whether the perimeter is a boundary, a
  shrine, a dump, or a gallows;
- **loss practice** — what is done when a raft parts, an island sinks, a crust gives, a host
  dies; where the memorial goes;
- **the record of scars** — whether old anchor marks are erased, read, honoured, or claimed
  as title; and
- **stranger law** — what a newcomer owes before they may cross.

**Two proposed first culture cards (both PROPOSED; build order, not exclusion):**

- **`CULT-KEEPERS`** — tenancy as *tithe to a keeper caste* who alone may touch anchors.
  The site's social spine is the tithe gate at the landing; the anchor field is sacred and
  legally untouchable; strangers buy passage in kind. *(Extrapolated from one rolled TIYL
  start, which placed a faction named **The Tithe-Keepers** in the same opening scene as the
  Dorsal Market — §8.3 explains why that is suggestive rather than proof. This card rests on
  a single faction name and no occupant study; it is the thinnest thing in this document.)*
- **`CULT-MAINTAINERS`** — tenancy as *claim by maintenance*: whoever adds reed, plank, or
  lashing to a patch owns it while they keep it up, and abandonment is forfeiture. The
  anchor field is common, contested, and constantly re-negotiated; the scar record **is**
  the land registry.

Each card must change a construction, workflow, ownership, or interaction fact. Palette and
motif support the read; they cannot carry it.

**No occupant card exists yet, and that is the site's largest research debt.** "Who lives
on a whale's back, and by what right" has not been studied. §15.4.

---

## 12. Occupancy, hooks, lighting and cutaway, world context, boundaries

### 12.1 Occupancy states

Tenanted and current · tenanted and in arrears · contested between two claimants (this is
...
Each must touch a real circuit and at least one map lever: a parted mooring · a tithe in
arrears · a maintenance strike · a stranger who does not know the clock · a body found in
the anchor field · a patch of substrate that has stopped behaving · a rival driving new
stakes · the host stirring early · a plank line cut in the night · a debt owed to the
ground itself · someone selling title to ground that will not exist at the next phase ·
salvage from a raft that parted last season · a newcomer who wants to *found* rather than
anchor.

### 12.3 Lighting

- Practical light on a Site-12 site is nearly always **carried or mounted on the anchored
  structure**, because there is nothing else to hang it from. Lamp positions therefore
  track the anchor field and the knot.
- Daylight over water, mat, or crust reads through the **margin**: the site's outline is
  most legible when the surround is bright and the site is not.
- The **underside** may be honestly black. Exposure, restrained ambient bounce, AO, contact,
  and honest reflection preserve nearby form. No unexplained lamp.
- Substrate-owned light (bioluminescence, luminous crust, a host's own glow) is licensed
  **only** by the rolled substrate facts and by THE CAUSAL LIGHT LAW inherited from Site 7:
  a light must have a cause in the world before it may have a position on the map.
- Reflected and refracted bounce off water, ice, or glass is a *legibility budget* the site
  gets for free and should spend deliberately — it is the one thing this site has that dry
  sites do not.

### 12.4 Cutaway

Most Site-12 expressions have **no overhead boundary at all** (deck, mat, flat, crust), so
cutaway is a non-issue for them. Where one exists — a hull's below-deck, a hive's interior,
a bubble-city's shell — the fixed camera omits the camera-facing overhead while showing
enough rim, beam, and back mass to make the enclosure believable.

**A Site-12-specific cutaway rule (PROPOSED):** the substrate's *underside* may be shown in
section at the margin even when it is not shown elsewhere, because "what is under the floor"
is a gameplay fact this site cannot afford to hide. One honest cut at the perimeter tells
the player the thickness, the material, and what is beneath.
...
The campaign framing is that Site 12 *is* the Strange/Volatile/Mythic end. Measured against
the engine's own bands, **that is true of the state door and false of the noun door.**

- **The noun door is ungated.** `place-master-setting` carries 15 substrate-anomaly rows
  across all five bands, **four of them Grounded** (Mire-End, Old Oak Wharf, The Drowned
  Port, The Ice-Road Town). `dungeon-type` — which owns the Living Hive and the
  Megastructure — has **no Band column at all**; it is a flat Fork-class d100, so a living
  dungeon is exactly as likely in a Grounded baseline region as at the rim.
- **The state door is gated, tightly and correctly.** The Becoming is reachable only from
  `pInternal` 20 (*Volatile*) or `pImpersonal` 12 (*Strange*); the Intrusion only from
  `pExternal` 20 (*Mythic*); the terminal transformation is `Impending Doom` 12 (*Mythic*).
  Both doors are Commitment-class tables, which is what `SPICE-CURVE.md` §4 requires of
  anything that may reach Volatile/Mythic.

> **PROPOSED grading rule.** A substrate's band is the **highest of three factors**:
> **AGENCY** (does the ground act?), **IMPOSSIBILITY** (does it require a law this world
> does not have?), and **STAKE** (is its failure scheduled and terminal?).
> **How strange it looks grades nothing.**

Tested row by row against the live bands of all 15 substrate rows in
`place-master-setting`, the rule agrees 15 of 15
(`Reference/Anomalous-Living-Mobile-Study-0727/synthesis.md` §3c). *The Fracture-Market* —
mundane geology, Volatile band — is the row that forced the STAKE factor, and it matches
`SPICE-CURVE.md`'s own definition of Volatile as *"reality strains; consequences escalate."*

Consequences for the generator:

- **a Site-12 roll may not raise its own band.** Building a market on a whale does not make
  the scene Volatile; the whale's agency already set it Strange when the place was rolled;
- **Grounded Site-12 sites must look ordinary** — a reed island rendered with uncanny
  lighting is a lie about its band; and
- **the transform door inherits the pressure's band and may not exceed it.**

### 12.7 Rolled world-context projection

Site 12's context problem is the inverse of every other site's. Other sites need context to
explain how their edges sit in the world; **Site 12 needs context to explain that its edges
do not sit in the world at all** — the FFT airship hull has no apron, the Uros mat has no
beach (`S12-A1`).

- The context apron must project the **surround** (the far bank, the mainland town, the
  reef, the sky, the host's wake) while adding **no traversable cell** at the margin.
- On a mobile site, the surround is the thing that changes while the site does not. A
  changed context roll must be able to move a mobile site's whole world without touching a
  single committed id — that is the site's best available proof of the projection contract.
- Forbidden visual claims: a painted shoreline that implies a second landing; a painted
  bridge, jetty, or gangway that is not a committed portal; painted ground beneath a mat or
  deck that contradicts the stated thickness.
- **No context receipt has been rolled for Site 12.** `GOLDEN-SITE-WORLD-CONTEXT-
  PROJECTION.md` requires one before the first proof; it is listed in §15.4.

---

## 13. Structure, mechanisms, materials, decals, props

### 13.1 Inherited structure (shared kit — mostly Sites 2, 5, 10)

Plank floors · posts and beams · piling and pile caps · terrain slabs and terraces · short
stairs and ramps · rope and lashing · shed and gable cover · doorframe sockets · water
surfaces · rock clusters · ordinary building frontage and market stalls.

**This is the finding that overturns the "biggest skin buy" assumption:** the great majority
of Site 12's first three rungs is timber, rope, plank, and piling the portfolio already owes
to other sites.

### 13.2 Site-owned structure (the actual invent list)

Substrate slab with a **stated thickness and an underside face** · mat/deck edge piece (the
margin) · plank-spine segment (1 cell, with a **removable** variant) · gangway/ramp with a
stowed state · stake and driven-pile anchor family with hold states · mooring bitt, cleat,
and line · pontoon/float mass · waterline and datum-line socket · piling-forest under-deck
frame · crust panel with a **breached** variant exposing subgrade · perimeter halo terrain ·
plate-and-gap terrain (the archipelago) · rim terrain (ground around a central void) ·
growing-joint anchor (living substrate) · anchor-scar decal socket · fast-point mast/post
family · marker post with a datum scale.

### 13.3 Mechanisms and interaction ownership
...
mooring line · waterline/datum socket · piling frame · fast-point post · marker post.

### 13.4 Material, decal, and prop demand
...
- **Props:** poles, gaffs, and boat hooks · coiled line and spare lashing · spare plank and
  reed bundles · maul and stake stock · bailing gear · a sounding line (the tool that reads
  the substrate) · floats and fenders · tally board at the tithe gate · lanterns with
  mountings · salvage from a parted raft · a marker post's spare crossbar.

Anything that changes collision, cover, route capacity, sight, support, load band, light
position, or interaction reach is geometry or a mechanism — not a decorative prop.

### 13.5 Stretch analysis — how far Site 12's resources reach

- **The plank spine, the piling frame, and the driven-stake anchor** serve Site 2 (camp
  decks and lashings), Site 5 (staging and shoring), Site 10 (waterfront frontage and market
  boardwalks), and the settled-life program's **Port / Harborfront** entry (§3.2) almost
  without modification. The port entry has no other structural owner.
- **The two-datum machinery** (a committed high state and low state over one id set) is
  the exact mechanism the settled-life program's **flooding/inundation** entry (§3.7) needs,
  and the mechanism the marathon's unhomed flood-site demand has been asking for across four
  sets.
- **The plate-and-gap and rim terrains** are pure subtraction from terrain the engine
  already builds, and they serve Site 7 (breakdown chambers, sinkholes), Site 5 (stope
  voids), and Site 9 (breached curtain and ditch).
- **The load-band relation** generalises to every site with a rotten floor, a weak bridge,
  or a strained roof — it is a portfolio-wide capability that only Site 12 has a reason to
  build. And per §8.2a it is mostly a *recovery* job, not a build job: 300 authored footing
  rows already carry limits, costs, and coverage geometries that no roller currently
  delivers.
- **The tenancy circuit** generalises to Site 8 (a third possible claimant), Site 6
  (custody as a form of forced tenancy), and every settled-life venue with a landlord.
- **Reskin reach:** the same raft-quarter chassis reskins to a barge-town, an ice-road
  market, a log-drive camp, a lake-village, a reef settlement, a hive gallery, and a
  dorsal market by changing material, anchor class, and host state — not geometry.

---

## 14. Runtime facts, stable ids, provenance, proof receipt
...
**Two id laws specific to this site:**

1. **Ids survive the datum.** An id committed in the high state persists in the low state
   even when unreachable. Two datum states are one place.
2. **Anchor scars are ids.** The record of former occupancy is retained data, not dressing —
   it is what a returning player reads, and at `CULT-MAINTAINERS` it is the land registry.

The proof receipt must additionally record: seed/version · `rollerLineage` ·
`sourceRollRefs` · committed grid · camera · standee envelopes · culture/tenancy cards ·
every operating-state cause · selected assemblies and provenance · clay/tactical/dressed
surface ids · capture ids · validation results · and any fallback used. A screenshot without
that receipt is a study image, not retained proof.

---

## 15. Proof, MVP, Ideal, remaining evidence, next demonstration

### 15.1 Proof

One deterministic `SUB-RAFT-01`: seven required zones · both datum states over one id set ·
a trusted path narrower than the site · an anchor field of at least four anchors with hold
states · a parted-and-recoverable raft · a fast point on the strongest anchor · a legible
margin with no apron · the underside as a tight interior · four standee envelopes with
load bands shown in the tactical overlay · clay / tactical / dressed captures over the same
ids · daylight, working-lamp, and unlit captures · **one anchor-cut state and one
anchor-restored state** · **one high-datum and one low-datum capture** · two changed seeds ·
one changed world-context receipt over an unchanged chassis · and one adversarial case
(a narrow envelope, a presentation-large standee on a 1-cell path, or a maximum-swing
datum). At least three viable plans demonstrated, **one of them non-combat** (plan 2, §7.2).
...
interiors · mobile sites under way with a changing surround · substrate-as-claimant under
Site 8 · disaster, salvage, and evacuation play · broad cultural evidence for tenancy forms.

### 15.4 Remaining evidence — the honest gap list
...
5. **No felt-register answer.** Every family in this study is a *felt* thing — the bog
   quakes, the deck rolls, the hull works, the whale breathes. Nothing here addresses how
   that reaches a player through a fixed camera and a static diorama, and it may be the
   site's real failure mode.
...
substrate.
*What stays unknown either way:* whether a floor that *decides* things reads as a place or
as a monster. That is why the whale is queued second and not deferred to Ideal.
*Easy to revise?* Yes — the order is a queue, not a commitment.
*Needs to judge it:* the five-expression family board and a side-by-side of the raft and
the whale over the same seven relations.

**FQ-2 — Grounded Site-12 sites should look completely ordinary.**
*The choice:* whether a floating reed village, a tidal port, or a boat mill gets any
uncanny treatment at all. Four of the fifteen substrate settlements in the live table are
Grounded, and the most-rolled world-origin in the TIYL batch is one of them.
*Recommendation:* ordinary. No uncanny lighting, no wrongness in the palette. The site is
strange in what it *asks of you*, not in how it looks — and per §12.6 appearance grades
nothing.
*What it changes later:* it makes the Strange rungs land much harder when they arrive, and
it means the site's first fixture will look like a nice riverside village, which may read
as underwhelming in a capture packet.
*Easy to revise?* Yes, entirely a dressing decision.
...
### 15.6 The next visible demonstration

A clay `SUB-RAFT-01` at both datums, captured as: fixed-camera plan-and-section hybrid with
the trusted path continuously readable · the anchor field labelled with hold states and hold
directions · one anchor-cut / anchor-restored pair showing the cluster swing · four standee
envelopes with load bands on the overlay · the underside as a separate tight interior ·
context-off and context-on over an unchanged chassis · and the three plans of §7.2 drawn
over the same ids.

Before that: close gap 1 (§15.4) by obtaining one measured section, and open the occupant
study.

---

## Appendix — research packet
