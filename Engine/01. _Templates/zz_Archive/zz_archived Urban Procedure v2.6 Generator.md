> [!info] Urban Procedure v2.6 Generator
...
> - **Posture-aware fallback chains:** undersized adventures degrade within their posture family before crossing (Investigative→Trail, Reactive→Rundown/Crucible, etc.). Stronghold→Layer Cake no-op fixed. Min segment input bumped to 2.
> - **DM Briefing rewrite:** three purpose-built paragraphs (Scene / Situation / Texture) replacing the old data-dump sentence. Now uses `typeAtmo`, `skinVis`, `originFlav`, and `restName`/`restDesc`.
> - **Heat starting states:** `HEAT_START` map for all 16 topologies. Displayed above the Heat table in the index note. Gauntlet=2, Rundown=3, Stronghold/Shell Game/Crucible/Tightrope/Ratchet=1, rest=0.
>
...
<%*
// =============================================================================
// URBAN SKETCH GENERATOR — Arcana Engine v0.25 · Urban Procedure v2.6
// v2.6: Label diversity overhaul. Cycle topologies inject breather nodes;
...
async function generateSegmentNote(segNum, node, exits, segNumbers, nodeMap, topoName, threatCtx, adventureName, tier, subRoll, dense, sceneFrame) {
...
for (const nodeId of bfsOrder) {
  ...
  let noteContent;
  if (node.isFinale) {
    noteContent = await generateFinaleNote(segNum, node, exits, segNumbers, nodeMap, resolvedTopo, threatCtx, adventureName, catalystResult, tier, dense, sceneFrame);
    segLocations[nodeId] = node.label;
  } else {
    const subRoll = await rollSubTable(node.label);
    noteContent = await generateSegmentNote(segNum, node, exits, segNumbers, nodeMap, resolvedTopo, threatCtx, adventureName, tier, subRoll, dense, sceneFrame);
...
*Generated with Arcana Engine v0.25 — Urban Procedure v2.6*
`;
