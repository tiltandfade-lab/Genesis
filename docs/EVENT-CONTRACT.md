...
| `prep_contact` | `{nodeId, enter?}` | declared (DM, the player enters a rumored frontier) | SESSION-PREP — `lockOnContact(w,p.nodeId)` locks the frontier to canon; `enter:true` also moves `w.currentNodeId`, stamps the departure day at the prior node (WORLD-TURN §1 T3), and fires `worldTurn(w,"revisit",...)` |
