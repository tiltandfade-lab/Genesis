> [!info] Urban Procedure v3.0 Generator
...
> - **`S{nn} — {label}.md`** — one note per segment. Contains location, sensory, encounter, dressing, transition, scene frame, optional extras, secret.
> - **`{name}.md`** — index note with DM Briefing, Setup table, topology diagram, segment roster, Heat rules, full content scan, and Prep Roster (NPC, creature, and battlemap lists).
>
> v3.1: **Prep Roster.** NPC Roster (contact type + hook + blank name), Creature Roster (grouped by stat block with segs), and Battlemap Prep List (every combat/hazard/problem segment + finale, with scene frame and dims).
>
> v3.0: **Feature-complete.** Scene frames, optional extras, tier-aware magic, unified d400 spatial table.
> - **Scene Frame d400:** single `Urban Scene Frame` table replaces four category-routed d100s. Every segment rolls on the same table regardless of label — streets, plazas, rooftops, and thresholds all in one pool. No routing logic, no label-to-category mapping.
> - **Optional section:** every segment and finale rolls Feature (Urban Feature d100), Active Magic (tier-aware), and Interactable Object (Urban Interactable Object d300). Printed under an **Optional** header — DM picks up what fits, ignores the rest.
> - **Tier-aware Active Magic:** T1 rolls on Urban Magic Effect Lv 1-5, T2 rolls on Urban Magic Effect Lv 6-10 (new table — 4th/5th level spells, layered wards, planar bleeds, district infrastructure).
> - **Urban Segment Path d300:** replaced d20 transit table with 300 diverse scene-based locations. Eliminates repetitive "you travel by boat/cart/tunnel" results.
>
> v2.6: **Polish pass** — fallback chains, briefing prose, heat starting states.
> - Posture-aware fallback chains, DM Briefing three-paragraph rewrite, HEAT_START map for all 16 topologies.
>
> v2.5.2: **Label diversity overhaul** — cycle/ramp helpers for all 16 topologies.

<%*
// =============================================================================
// URBAN SKETCH GENERATOR — Arcana Engine v0.25 · Urban Procedure v3.1
// v3.1: Prep Roster — NPC Roster, Creature Roster, Battlemap Prep List added
//         to index note. buildEncounter() returns npcData/creatureData; finale
//         returns { content, npcData, creatureData }; encounter hoisted to loop.
// v3.0: Feature-complete. Scene Frame d400, Optional extras (Feature, Active
//         Magic, Interactable Object), tier-aware magic tables, d300 path table.
// =============================================================================
...
// ─── SCENE FRAME ───────────────────────────────────────────────────────────
// Single d400 table covers all urban spatial types (streets, plazas, rooftops,
// thresholds). Every segment rolls on the same table regardless of label.

// Encounter types that get full scene frame (name + dims + tactical feature)
const FRAME_FULL = new Set(["Enemy", "Hazard", "Problem"]);
// Encounter types that get light scene frame (name + dims only)
const FRAME_LIGHT = new Set(["Social", "Commerce", "Spectacle"]);
// Everything else (Rumor, Empty, Boon) skips scene frame

async function pickSceneFrame() {
  const rows = await readTable("Urban Scene Frame");
  if (!rows.length) return { frame: "[Scene?]", dims: "[Dims?]", tactical: "[Feature?]" };
  const row = rnd(rows);
  return { frame: (row[1]||"").trim(), dims: (row[2]||"").trim(), tactical: (row[3]||"").trim() };
}

function formatSceneFrame(sceneFrame, encounterType) {
  if (!sceneFrame || !sceneFrame.frame) return "";
  if (FRAME_FULL.has(encounterType)) {
    return `**Scene Frame:** ${sceneFrame.frame} — ${sceneFrame.dims} | ${sceneFrame.tactical}`;
  }
  if (FRAME_LIGHT.has(encounterType)) {
    return `**Scene Frame:** ${sceneFrame.frame} — ${sceneFrame.dims}`;
  }
  return ""; // Rumor, Empty, Boon — skip
}
  ...
  if (branch === "Enemy") {
    if (Math.random() < 0.75) {
      ...
      if (compName === "Factional Conflict") {
        ...
        return { type: "Enemy", text: `**Encounter: Enemy** *(Faction Clash)*\n- Faction A: *${fA}*\n- Faction B: *${fB}*\n- *${compT}*`, isEnemy: true, npcData: null, creatureData: null };
      }
      const slots = SLOT_MAP[compName] || ["low"];
      const creatureEntries = slots.map(slot => {
        let creature, crSlot;
        if (slot === "low")  { creature = pickFromPool(threatCtx.low);  crSlot = "Low CR"; }
        if (slot === "mid")  { creature = pickFromPool(threatCtx.mid);  crSlot = "Mid CR"; }
        if (slot === "boss") { creature = pickFromPool(threatCtx.boss); crSlot = "Boss CR"; }
        return { crSlot, creature };
      });
      const creatureLines = creatureEntries.map(e => `- ${e.crSlot}: *${e.creature}*`);
      return { type: "Enemy", text: `**Encounter: Enemy** *(${threatCtx.id} — ${compName})*\n- ${compRoster} | *${compT}*\n${creatureLines.join("\n")}`, isEnemy: true, npcData: null, creatureData: creatureEntries };
    } else {
      const [enemy] = await pick("Urban Enemy Category", 1);
      return { type: "Enemy", text: `**Encounter: Enemy** *(Complication)*\n- *${enemy}*`, isEnemy: true, npcData: null, creatureData: null };
    }
  }
...
async function generateSegmentNote(segNum, node, exits, segNumbers, nodeMap, topoName, adventureName, tier, subRoll, dense, sceneFrame, encounter) {
  ...
  const frameStr      = formatSceneFrame(sceneFrame, encounter.type);
  const secret        = await buildSecret();

  // Optional extras — Feature, Active Magic, Interactable Object
  const [featName, featDims, featDesc]  = await pick("Urban Feature", 1, 2, 3);
  const magicTable = tier === "T2" ? "Urban Magic Effect Lv 6-10" : "Urban Magic Effect Lv 1-5";
  const [magicClass, magicEffect]       = await pick(magicTable, 1, 2);
  const [objName, objPrimary]           = await pick("Urban Interactable Object", 1, 2);
...
**Optional**
- *Feature:* ${featName} — ${featDims}. ${featDesc}
- *Active Magic:* ${magicClass} — ${magicEffect}
- *Object:* ${objName} *(${objPrimary})*
...
async function generateFinaleNote(segNum, node, exits, segNumbers, nodeMap, topoName, threatCtx, adventureName, catalystResult, tier, dense, sceneFrame) {
  const [sensory] = await pick("Urban Sensory", 1);
  const [d1]      = await pick("Urban Set Dressing", 1);
  const [c1]      = await pick("Urban Set Dressing Condition", 1);
  const [d2]      = await pick("Urban Set Dressing", 1);
  ...
  const conns     = buildConnections(exits, segNumbers, nodeMap, dense);
  const topoField = TOPOLOGY_FIELDS[topoName] ? TOPOLOGY_FIELDS[topoName](node) : "";
  // Finales always get full scene frame — climactic scenes need spatial data
  const frameStr   = sceneFrame.frame ? `**Scene Frame:** ${sceneFrame.frame} — ${sceneFrame.dims} | ${sceneFrame.tactical}` : "";
  ...
  let finaleBossCreature = null, finaleNpcContact = null;

  if (finaleTrack === "Combat") {
    const [bossArch] = await pick("Urban Boss", 1);
    const [tacticalSetup] = await pick("Urban Tactical Setup", 1);
    finaleBossCreature = pickFromPool(threatCtx.boss);
    const bossCreature = finaleBossCreature;
...
**Reward:** ${reward}
`;
  const npcData = finaleNpcContact ? { entity: finaleNpcContact, hook: "Key NPC — Finale" } : null;
  const creatureData = finaleBossCreature ? [{ crSlot: "Boss CR", creature: finaleBossCreature }] : null;
  return { content, npcData, creatureData };
}
...
const segFullBlocks = [];
const segLocations  = {};
const prepNPCs       = [];  // { segNum, entity, hook, isFinale }
const prepCreatures  = [];  // { segNum, crSlot, creature, isFinale }
const prepBattlemaps = [];  // { segNum, location, frame, dims, isFinale }
...
for (const nodeId of bfsOrder) {
  ...
  // Roll scene frame per segment (contextual by label)
  const sceneFrame = await pickSceneFrame();

  let noteContent;
  if (node.isFinale) {
    const finaleResult = await generateFinaleNote(segNum, node, exits, segNumbers, nodeMap, resolvedTopo, threatCtx, adventureName, catalystResult, tier, dense, sceneFrame);
    noteContent = finaleResult.content;
    segLocations[nodeId] = node.label;
    // Collect finale prep data
    if (finaleResult.npcData) prepNPCs.push({ segNum, entity: finaleResult.npcData.entity, hook: finaleResult.npcData.hook, isFinale: true });
    if (finaleResult.creatureData) for (const cd of finaleResult.creatureData) prepCreatures.push({ segNum, crSlot: cd.crSlot, creature: cd.creature, isFinale: true });
    prepBattlemaps.push({ segNum, location: node.label, frame: sceneFrame.frame || "—", dims: sceneFrame.dims || "—", isFinale: true });
  } else {
    const subRoll = await rollSubTable(node.label);
    const encounter = await buildEncounter(resolvedTopo, threatCtx);
    noteContent = await generateSegmentNote(segNum, node, exits, segNumbers, nodeMap, resolvedTopo, adventureName, tier, subRoll, dense, sceneFrame, encounter);
    segLocations[nodeId] = subRoll ? subRoll.segType : "—";
    // Collect segment prep data
    if (encounter.npcData) prepNPCs.push({ segNum, entity: encounter.npcData.entity, hook: encounter.npcData.hook, isFinale: false });
    if (encounter.creatureData) for (const cd of encounter.creatureData) prepCreatures.push({ segNum, crSlot: cd.crSlot, creature: cd.creature, isFinale: false });
    if (FRAME_FULL.has(encounter.type) && sceneFrame.frame) {
      const bmLocation = (segLocations[nodeId] && segLocations[nodeId] !== "—") ? segLocations[nodeId] : (node.label || "—");
      prepBattlemaps.push({ segNum, location: bmLocation, frame: sceneFrame.frame, dims: sceneFrame.dims, isFinale: false });
    }
  }
  ...
  const sceneShort = sceneFrame.frame || "—";
  segIndexRows.push(`| [[${segFileName.replace(".md", "")}\\|S${String(segNum).padStart(2,"0")}]] | ${node.label || "—"} | ${segLocations[nodeId] || "—"} | ${sceneShort} | ${neighborStr} |`);
}
...
function prepSegLabel(sn, isFinale) {
  return isFinale ? `S${String(sn).padStart(2,"0")} (Finale)` : `S${String(sn).padStart(2,"0")}`;
}

// NPC Roster rows
const prepNPCRows = prepNPCs.length
  ? prepNPCs.map((n, i) => `| ${i+1} | ${prepSegLabel(n.segNum, n.isFinale)} | *${n.entity}* | ${n.hook} | — |`).join("\n")
  : "| — | — | *(none rolled)* | — | — |";

// Creature Roster — group by stat block name, collect all segs where it appears
const creatureMap = {};
for (const c of prepCreatures) {
  if (!creatureMap[c.creature]) creatureMap[c.creature] = { crSlot: c.crSlot, segs: [] };
  creatureMap[c.creature].segs.push(prepSegLabel(c.segNum, c.isFinale));
}
const prepCreatureRows = Object.keys(creatureMap).length
  ? Object.entries(creatureMap).map(([creature, data]) =>
      `| *${creature}* | ${data.crSlot} | ${data.segs.join(", ")} | — |`).join("\n")
  : "| *(none rolled)* | — | — | — |";

// Battlemap Prep List rows
const prepBattlemapRows = prepBattlemaps.length
  ? prepBattlemaps.map(b => `| ${prepSegLabel(b.segNum, b.isFinale)} | ${b.location} | ${b.frame} | ${b.dims} |`).join("\n")
  : "| — | — | *(none needed)* | — |";
...
| Seg | Label | Location | Scene | Connections |
|:----|:------|:---------|:------|:------------|
${segIndexRows.join("\n")}
...
| # | Seg | Contact Type | Hook | Name |
|:--|:----|:-------------|:-----|:-----|
${prepNPCRows}

### Creature Roster

| Stat Block | CR Slot | Appears In | Count |
|:-----------|:--------|:-----------|:------|
${prepCreatureRows}

### Battlemap Prep List

| Seg | Location | Scene Frame | Dims |
|:----|:---------|:-----------|:-----|
${prepBattlemapRows}
...
**At the Table**
[[Urban Sensory]] · [[Urban Set Dressing]] · [[Urban Set Dressing Condition]] · [[Urban Enemy Category]] · [[Urban Enemy Composition]] · [[Urban Contact]] · [[Urban Spectacle]] · [[Urban Rumor Intel]] · [[Urban Problem]]

**Optional Extras**
[[Urban Scene Frame]] · [[Urban Feature]] · [[Urban Magic Effect Lv 1-5]] · [[Urban Magic Effect Lv 6-10]] · [[Urban Interactable Object]]

---

*Generated with Arcana Engine v0.25 — Urban Procedure v3.1*
`;
