...
function fillOracleResult(){
  ...
  host.innerHTML=`<div style="border:1px solid var(--gold);border-radius:10px;background:#241d15;padding:12px 14px;margin-bottom:14px">
    <div style="font-size:11px;color:var(--ink-dim);letter-spacing:.06em;text-transform:uppercase">${r.id} · rolled ${r.dice} → ${r.total}${r.band?" · "+r.band:""}</div>
    <div style="font-size:15px;color:var(--bone);margin-top:5px">${r.text||"(no text)"}${s?` <span style="font-size:9px;color:${s[1]};border:1px solid ${s[1]};border-radius:6px;padding:0 4px;letter-spacing:.08em">${s[0]}</span>`:""}</div>
...
function fillOracleList(){
  ...
  host.innerHTML=Object.keys(byDom).sort().map(dom=>`<div style="margin-bottom:10px"><div style="font-size:11px;color:var(--gold-soft);letter-spacing:.06em;text-transform:uppercase;margin-bottom:4px">${dom}</div>`+
...
function renderOracle(){
  ...
  if(!ids.length){host.innerHTML=`<div class="section"><h3>The Oracle</h3>
    <div style="color:var(--ink-dim);font-size:13px;line-height:1.7">No compiled tables loaded yet. Generate them:<br>
  ...
  host.innerHTML=`<div class="section"><h3>The Oracle <span style="color:var(--ink-dim);font-size:11px;letter-spacing:0;text-transform:none">${ids.length} compiled tables · roll any of them</span></h3>
