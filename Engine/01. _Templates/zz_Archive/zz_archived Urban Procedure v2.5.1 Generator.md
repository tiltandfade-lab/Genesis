> [!info] Urban Procedure v2.5.1 Generator
...
>
> v2.5.1: Bug fixes — segment cap removed (was 8, now 30), sub-table deduplication, richer encounter display (Empty/Social/Hazard/Problem), DM Briefing cleaned up.

<%*
// =============================================================================
// URBAN SKETCH GENERATOR — Arcana Engine v0.25 · Urban Procedure v2.5.1
// v2.5.1: Bug fixes — segment cap 8→30, sub-table dedup, richer encounters,
//         DM Briefing cleanup. Full cascade:
//         Topology → Graph → Segment Types → Transitions → Encounters.
// =============================================================================
...
const TOPOLOGY_FALLBACK = {
  "The Fracture": "The Double Cross",
  "The Double Cross": "The Trail",
  "The Tightrope": "The Web",
  "The Ratchet": "The Rundown",
  "The Mosaic": "The Trail",
  "The Shell Game": "The Trail",
  "The Stronghold": "The Layer Cake",
  "The Gauntlet": "The Trail",
  "The Rundown": "The Trail",
  "The Carousel": "The Trail",
  "The Stakeout": "The Trail",
  "The Crucible": "The Turf",
  "The Layer Cake": "The Web",
  "The Turf": "The Web",
  "The Web": "The Trail",
};
...
const TOPOLOGY_LABELS = {
  ...
  "The Mosaic":       { seq: ["Fragment", "Fragment", "Fragment", "Fragment", "Fragment", "Fragment", "Fragment"], finale: "Revelation" },
  ...
  "The Gauntlet":     { seq: ["Start", "Waypoint", "Waypoint", "Waypoint", "Waypoint", "Waypoint"], finale: "Haven" },
  "The Rundown":      { seq: ["Spark", "Escalation", "Escalation", "Escalation", "Escalation"], finale: "Breaking Point" },
  "The Carousel":     { seq: ["Arrival", "Event Scene", "Event Scene", "Event Scene", "Event Scene"], finale: "Climax" },
  "The Stakeout":     { seq: ["Setup", "Watch", "Watch", "Incident", "Incident", "Incident"], finale: "Payoff" },
  "The Crucible":     { seq: ["Containment", "Pressure", "Pressure", "Pressure", "Pressure"], finale: "Release" },
  "The Double Cross": { pathA: "Path A", pathB: "Path B", opening: "Opening", convergence: "Convergence", finale: "Finale" },
  "The Tightrope":    { seq: ["Introduction", "Faction Scene", "Faction Scene", "Faction Scene", "Faction Scene", "Faction Scene", "Faction Scene"], finale: "Reckoning" },
...
function buildMosaicGraph(n) {
  // Emergent graph: fragments start isolated, connected through finale
...
function buildShellGameGraph(n) {
  // Shifting nodes: linear chase with temperature labels
...
function buildStrongholdGraph(n) {
  // Fortified hub: layers leading inward, like Layer Cake but with defense framing
...
function buildGauntletGraph(n) {
  // Inverted convergence: start with branches that merge toward haven
...
function buildRundownGraph(n) {
  // Cascade: linear, no backtracking, escalating labels
...
function buildCarouselGraph(n) {
  // Loop + tail: event scenes in a loop, climax branches off
...
function buildStakeoutGraph(n) {
  // Timeline spine: linear with branching decision points off Watch nodes
...
function buildTightropeGraph(n) {
  // Weighted mesh: introduction → faction scenes with cross-connections → reckoning
...
function buildRatchetGraph(n) {
  // Contracting: many connections early, few late
...
async function generateSegmentNote(segNum, node, exits, segNumbers, nodeMap, topoName, threatCtx, adventureName, tier, subRoll) {
...
async function generateFinaleNote(segNum, node, exits, segNumbers, nodeMap, topoName, threatCtx, adventureName, catalystResult, tier) {
  const [sensory] = await pick("Urban Sensory", 1);
  const [d1]      = await pick("Urban Set Dressing", 1);
  const [c1]      = await pick("Urban Set Dressing Condition", 1);
  const [d2]      = await pick("Urban Set Dressing", 1);
...
const segCount = Math.max(1, Math.min(30, parseInt(segStr) || 4));
...
for (const nodeId of bfsOrder) {
  ...
  let noteContent;
  if (node.isFinale) {
    noteContent = await generateFinaleNote(segNum, node, exits, segNumbers, nodeMap, resolvedTopo, threatCtx, adventureName, catalystResult, tier);
    segLocations[nodeId] = node.label;
  } else {
    const subRoll = await rollSubTable(node.label);
    noteContent = await generateSegmentNote(segNum, node, exits, segNumbers, nodeMap, resolvedTopo, threatCtx, adventureName, tier, subRoll);
...
const briefing = `In the ${skinName.toLowerCase()} ${typeArch.toLowerCase()} shaped by ${originCat.toLowerCase()}, ${catalystResult.toLowerCase()}. The local threat is **${threatCtx.id}** (minions: ${threatCtx.low}; enforcers: ${threatCtx.mid}; boss: ${threatCtx.boss}). The cultural motif of ${motifName.toLowerCase()} (${motifDesc.toLowerCase()}) is ${modName.toLowerCase()} — ${modDesc.toLowerCase()}.`;
...
*Generated with Arcana Engine v0.25 — Urban Procedure v2.5.1*
`;
