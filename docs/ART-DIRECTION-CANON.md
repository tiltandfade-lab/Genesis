...
## ONE PROCEDURAL VIGNETTE ENGINE + GOLDEN PROGRAM (Adam, 2026-07-29 — binding; verbatim)

> "ok so they cannot and should not be two separate engines, they are just two functions of the same engine"

> "ok, this is excellent, this is our next big plan and move. Take a deep look at the golden site information, make sure that they are organized in such a way that it feeds this plan to build our procedural engine. The golden sites cover almost EVERYTHING that the game needs to be able to generate to accomplish the vignette/diorama vision. So we start small and simple, and determine what assets need to be generated in-engine, and what assets need to be generated in meshy, and what assets can be generated by sprite extrusion, what textural assets need to be generated etc... we need a multi wave plan and this will be THE ULTIMATE PLAN for the next several weeks which we only run multi-sessions when they serve the golden site engine right? running the golden site plan should also include intermittent battle simulations between PCS and enemies
>
> do you think running intermittent game testing should also happen? like run a huge census of walk rolls to make sure we are still getting solid coverage of everything we need? I assume at some points we will need to modify our walks to actually harness the things we are creating in these golden sites"

Binding consequences:

- Terrain formation and building/site construction are stages of one Procedural Vignette
  Synthesizer. They share the story request, macro topology, elevation/support field,
  connections, tactical reservations, asset resolver, receipts, and projections. Separate modules
  are allowed; rival terrain and building generators are not.
- `GOLDEN-SITE-PROCEDURAL-VIGNETTE-MASTER-PLAN.md` is the next several-week orchestration owner.
  The twelve Golden Sites remain coverage proofs rather than runtime enums; ordinary venues and
  apparent census gaps use the same host/transform/substrate/compiler path.
- The asset source follows shape and gameplay need. Exact terrain, structure, sockets, collision,
  and tactical envelopes remain engine-owned; Meshy supplies admitted complex-volume donors;
  sprite extrusion/faced boxes supply shallow identity and ordinary furnishings; material tools
  supply surfaces and causal state. A semantic request does not hard-code its production source.
- Walk-demand census, intermittent real game testing, and PC-versus-enemy battle simulation are
  continuous engine evidence, not optional cleanup after visual completion.
- Walks may be modified when retained evidence shows a missing adapter, receipt, typed hint, host,
  transform, substrate, or genuinely absent story demand. They may not be rewritten merely to
