...
| Land-travel, route, transport, and macro-biome continuity | Every journey endpoint is a canonical node; one shared TravelWalk/SceneLineage contract owns departure/cursor/clock/arrival/turnback while route baseline canon remains distinct from journey-specific time; a small rational macro-biome field refines locally without contradicting known geography; early canonical transport possession/eligibility precedes prose or mechanical mounted travel; development may use fact-locked prose-only transport, but playable pre-alpha distinguishes foot, mounted wilderness, road, and wagon through typed route/transport/pace/exposure inputs | C2H | C2I/C2J | C4E specialized per-mode assemblers, richer probability matrices, generated road topology/conditions, and deep mount/vehicle ecology over the common contract | Correct shared travel becomes repetitive or implausible by mode; route/world scale exceeds the retained macro/refinement constraints; transport condition/economy becomes play-critical | MAPPED IN PART; F10.9g.1-g.5 accepted, generated substrate/choice/asset questions active |
...
- P10.9 F10.9a accepted the transactional SceneLineage handoff and added C2D as the first retained continuity proof;
  ...
  governed same-scene attention, while Waves 6-7 still own detailed behavior. F10.9g.1-g.5 accept one shared staged
  TravelWalk contract before specialized assemblers, baseline-route versus journey-time separation, an exposure
  opportunity matrix, prose-only transport as a development scaffold before typed pre-alpha effects, early
  canonical mount possession/ability, and coherent macro-biomes that refine locally and disorder toward the Fray;
  C2H-J/C4E own the staged proof/promotion path. F10.9h accepts bounded canonical town fabric before scoped
  contiguous-town enrichment. Generated P10.9 follow-ups still own the travel substrate/matrix/details, town,
  CrisisChain, and broader recovery/visual-memory obligations before its MVP gate can be complete.
