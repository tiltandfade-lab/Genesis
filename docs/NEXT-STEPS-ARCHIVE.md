...
# Genesis — Next Steps Archive

Superseded queue history split out of `NEXT-STEPS.md` on 2026-07-09 to keep the live file cheap
to load every session. Each block below was self-declared superseded by a later block in its own
text ("supersedes below" / "SUPERSEDES the above") — this is a read-only record, not a place to
add new entries. `CHANGELOG.md` remains the authoritative decision/landing history; this archive
just preserves the old ordered-queue framing for anyone reconstructing why a call was made.
...
4. ☐ **A live playtest soak** (bridge, Adam at the keys) on the post-production world — spicy baseline, scene-risk telegraphs,
   ...
   drives build order).
5. ☑ **Small fixes queue — DONE 2026-07-07 (later), the HQ2 wave:** founding digest 36KB→7KB
