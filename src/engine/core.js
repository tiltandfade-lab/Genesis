...
function lookup(name){const tbl=T[name],r=rollDie(tbl.die);let idx=tbl.rows.findIndex(x=>r>=x[0]&&r<=x[1]);if(idx<0)idx=0;const row=tbl.rows[idx];return {roll:r,idx,cat:row[2],name:row[3],desc:row[4],frag:fragAt(name,idx)};}
