/* dev/model-qa/probe-capture.mjs — one-off headless shot of whole-body-probe.html.
...
await page.goto(`http://127.0.0.1:${PORT}/dev/model-qa/whole-body-probe.html`, { waitUntil: "networkidle0", timeout: 20000 });
await new Promise(r => setTimeout(r, 700));
const stat = await page.$eval("#stat", el => el.textContent).catch(() => "(no stat)");
const buf = await page.$("#stage").then(h => h.screenshot());
fs.writeFileSync(path.join(__dirname, "probe.png"), buf);
