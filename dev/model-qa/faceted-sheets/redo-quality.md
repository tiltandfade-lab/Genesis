...
status: fireable
created: 2026-07-14
regenerated: 2026-07-14 (post worktree failure)
program: faceted-regeneration
---

# RQ — Quality re-do batch (2026-07-14 QA pass, 510 files reviewed)

Every identity whose ONLY candidate(s) failed QA: style drift off the triangulated facet
contract, crops, split-tail/limb-count anatomy, white/scene backgrounds — or whose file contains
the WRONG SUBJECT entirely (the F4/F8 misrouting). Same architecture as `refire-r*.md` — paste
[`SHEET-TEMPLATE.md`](SHEET-TEMPLATE.md) first, then one sheet section per call.
...
mirrored second tail tip at bottom-center; wyverns are BIPEDAL; every wing joins the body once.


## F2 re-dos (2 identities)

Failed files (reason digest):
- `spr-fantasy-kobold-inventor-halfling-orchard-keeper-elderly-halfling-knows-every-tree-in-the-grove-by-name-candidate-001.png` — style-drift:soft-facet
- `spr-fantasy-red-dragon-wyrmling-white-dragon-wyrmling-candidate-001.png` — cropped; bg-rekey:pale-pink; note:red wyrmling's left wingtip clipped at left image edge; white wyrmling's rig
