...
- **Diagnose each real red: regression vs staled fixture.** A frozen snapshot / git-show reference / size
  ...
  load-bearing. **Never weaken the assertion to go green** (CLAUDE.md "Validators preserve the thing's job").
...
touched — a one-paragraph close so the next session (or the next Claude) can pick up cold.
