...
1. **Dormancy is a factorized state, not an adjective.** The literature gives three
   independent factors that produce every case in the sweep: *how the operators left* (exit
   mode), *how long ago* (elapsed band), and *who came after* (reclaimers) — applied to the
   host's own circuits. Decay is only one of six vectors; the other five (robbed, flooded,
   buried, overgrown, sealed) are not decay at all, and two of them **preserve** the site
   rather than degrade it.
2. **The funerary bucket is a host program, not a state.** The census mapped Subterranean
...
3. **Site 3's build is a coherence layer, not a ruin kit.** The engine already rolls tiered
   ...
   thresholds, flooded chambers with dry islands, four kinds of pitch black, and a whole
   empty-result table about reading a dead place. What it cannot do is make those rolls agree.
   A handful of committed causal facts, plus biasing and validation against them, buys more
   than any new geometry would.

## The load-bearing laws this packet proposes

All `PROPOSED`. None is a founder ruling.
...
> Deliberate closure is a distinct noun built in a *cheaper, later* material than the opening
> it fills, and it changes access without changing structure.

Image D3-01: a dressed gothic arch filled with flint-and-brick infill. Cheap geometry
(one flat panel in an existing socket), enormous narrative payload, and it is the tell that
separates *reduced* from *ruined*.

### THE EVIDENCE-OF-EXIT LAW

> What was taken versus what was left is the site's primary information layer, and it is
> readable before any combat.

Schiffer's curate behaviour / de facto refuse / draw-down, rendered as a per-object flag.
The rusted drum in D5-01 is the canonical case: too heavy and too cheap to carry out.

### THE COVER-CLASS BOUNDARY

> When a wall drops below roughly one metre, the interior stops being an interior. The volume
> obeys outdoor rules and the wall becomes half cover.

Images D2-01 (canopy inside the wall lines, wall reduced to a kerb) and D5-02 (waist-height
block stubs on open ground). This is the single hardest tactical consequence of the ladder,
and it is why the ladder's rungs must be per-zone rather than per-site.

## The one-page ladder

| rung | what happened | what changes in play |
|---|---|---|
| **D0 held** | maintenance stopped, site actively kept: locked, dry, greased | nothing spatial. Contents thinned. No light owner on site but an *owner* still exists — this is a social and legal problem, not a physical one |
| **D1 leaking** | envelope breached at one point | a water path, a soft floor patch, one new sight/sound line; still fully reversible |
| **D2 unroofed** | cover gone over one or more bays | the room becomes weather-exposed; sky light and rain apply; walls stand; openings survive as voids |
| **D3 partial collapse** | wall or vault sections fail | a rubble cone forms *inside* against the wall it fell from, giving a ramp; new breach routes and new blockages; upper floors gone, only the scar band remains |
| **D4 reduced** | walls below standing height | interiors become exteriors; walls become half cover; vegetation follows the wall lines |
...
| concept doc (2026-07-25/26) | research verdict |
|---|---|
...
## Honest gate reading after this packet

- `RESEARCHED` — the three depth-law limbs are served: a sourced breadth sweep, twelve
  downloaded and individually read images with a per-image ledger, and an FFT comparison with
  its corpus narrowing declared. Four substantive gaps remain declared (non-European
  construction, no measured decay rates, no robbing or mothball image, systematic FFT cohort
  not re-verified). **PARTIAL is the honest value, not PASS.**
- `FOUNDER-RULED` — **OPEN.** Every law above is `PROPOSED`. Three founder questions are
  outstanding.
- `BRIEF-CONGRUENT` — **PARTIAL** once the working spec lands, because founder proposals
  still carry load.
- `CLAY-PROVED` — **OPEN.** Nothing here is a rendered fixture and nothing in this packet can
  ever earn that gate.
